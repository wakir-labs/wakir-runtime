...
| Component | Status | Notes |
|---|---|---|
| WAT spool format (`docs/wat-spool-spec.md`, leaf projection in `wirelang/specs/wat-leaf-projection.md`) | `stable-ish` | Frozen by the hash specification and by `tests/fixtures/proof-path-vectors/`; a change to the leaf projection changes the Merkle root of every recorded test vector, which is loud by construction. |
| Merkle aggregator (`wat/merkle/aggregator.py`, `wat/cmd/aggregator_cli.py`) | `stable-ish` | The core of the claim. Hash rules in `docs/wat-hash-spec.md`, cross-checked by `tests/wat/test_hash_consistency.py`, which since the Welle-3 lane promotion runs on every pull request in the `Lane — WAT core` job of `runtime-acceptance-gates.yml` (its result is folded into the required `runtime acceptance gates` context), and — this is the part CI actually runs — re-derived independently by `wakir-verify` in step 5 of the proof path, and re-derivable from the real hour cohorts in `tests/fixtures/wat-tv2-real/` and `wat-tv3-real/`. |
...
| External-verifier conformance harness (`tooling/external-verifier-ajv/`, `tooling/external-verifier-hyperjump/`, `docs/external-verifier-conformance.md`) | `beta` | Four validator poles — `python-jsonschema`, `fastjsonschema`, Node.js `ajv` and Node.js `@hyperjump/json-schema` — validate our published schemas against the same vector set and must reach the same verdict per vector. As of the Welle-3 lane promotion this is **executed**: the `Lane — WAT core` job of `runtime-acceptance-gates.yml` installs Node 22 and both harnesses via `npm ci` from committed lockfiles, installs `fastjsonschema`, hard-fails if any pole is missing, and runs `tests/wat/test_external_verifier_parity.py` (driven by `scripts/external_verifier_validation.py`) with its result folded into the required `runtime acceptance gates` context. It stays `beta` because the first real execution was 2026-09-15: the claim is now checked, but it has not been checked for long. Corrected three times: on 2026-09-14 this row credited `external-verifier-drift.yml`, a workflow that never touched `tooling/` and no longer exists; the correction that replaced it named a test module no workflow ran, and no workflow installed Node at all, so the Node halves had never executed once. |
...
| Component | Status | Notes |
|---|---|---|
| SPIFFE/SPIRE workload identity (`wirelang/adapters/real_spiffe_workload_api.py`, `wirelang/identity/svid_workload_identity_canonical.py`, `infra/spire/`) | `experimental` | Server and agent setup are documented (`docs/spire-server-phase-2-1.md`, `docs/spire-agent-phase-2-2.md`, `docs/spire-trust-bundle-rotation.md`) and the Python client has a graceful fallback path. The live path was `.github/workflows/live-vm-acceptance.yml` until 2026-09-22, when it was withdrawn under ADR-0077: measured against the Actions API it had never run, and it could not have — a hosted runner has no route to a private-network target and this repository holds zero Actions secrets. The live path is now a single one: the operator-hand run on the substrate node. It has happened, on 2026-09-21/22, and what it found is in the row below. There is no standing gate and no CI-side artefact. |
...
| Container and deployment surface (`quadlet/`, `compose/`, `infra/`, `scripts/install-persona-quadlet.sh`) | `experimental` | Quadlet units, network and volume definitions, cosign-verified images and a bring-up path exist and are lint-checked; end-to-end bring-up runs through `e2e-bringup-ci.yml` on a narrow path filter. The real virtual-machine leg of `e2e-vm-acceptance-gate.yml` was withdrawn on 2026-09-22 under ADR-0077. It had never run — of that workflow's 71 runs to 2026-09-14 none was a `workflow_dispatch` — and the reason turned out to be structural rather than habitual: the job selected a self-hosted runner label, and this repository has no self-hosted runners, so a dispatch would have queued rather than failed. It read `skipping` on every pull request while the workflow reported success from its hermetic job. That hermetic job remains and is all this workflow now claims. Nothing here is continuously proven. |
...
| Continuous-integration gate set (40 workflows, collecting job inside `.github/workflows/runtime-acceptance-gates.yml`) | `stable-ish` | The gates are the most reliably exercised component in the repository — they run on every pull request, and their configuration is itself tested (`tests/ci/`, `tests/workflows/`, `tooling/ci/verify_branch_protection_required_checks_doc.py`). All thirteen contexts read live on 2026-09-14 are active in branch protection (`docs/ci/branch-protection-required-checks.md` §1); the earlier "nine of twelve, three await an operator step" was documentation drift — the operator had activated them, and `secret-scan` had never been inventoried. Enforcement is `non_admins` (`enforce_admins: false`). The cross-workflow aggregator that used to be named here was retired on 2026-09-15 under ADR-0075: it ran 865 times, was never a required context, polled 6 of the 13, and its own documentation claimed it was the required status. Aggregation now happens inside a single workflow, where `needs.*.result` is inherited rather than polled (`docs/ci/retired-workflows.md`). |
...
| Live virtual-machine acceptance path (`scripts/federation-live-vm-acceptance.sh`, `scripts/ci-live-vm-*`) | `internal` | Requires operator credentials and a provisioned host, so it cannot run for an outside contributor. **There is one way to live evidence here, not three.** Until 2026-09-22 this row named three: `live-vm-acceptance.yml`, the real-VM leg of `e2e-vm-acceptance-gate.yml`, and the operator hand. The first was never dispatched in four months and could not have worked if it had been — hosted runner, private-network target, zero Actions secrets. The second selected a self-hosted runner label that no runner in this repository carries, so it could not be scheduled at all and its absence read as a skip beside a passing workflow. Both were withdrawn under ADR-0077; the earlier reading here attributed their silence to missing dispatches, which was the charitable half of the explanation and not the operative one. What remains is the operator-hand run on the substrate node. It works: three runs on 2026-09-21/22 surfaced four defects that had looked green for four months, and the substrate failed the acceptance (5/8 and 4/8 smoke checks). It is a handled action with a deadline rather than a gate — the `live-vm-operator` exemption in `tests/lanes/lane_assignment.json` carries a review date checked against the calendar, and the lane goes red when it lapses. Read this row as: exercised, failing, dated. Not as covered. |
