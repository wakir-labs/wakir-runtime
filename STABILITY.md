...
| External-verifier conformance harness (`tooling/external-verifier-ajv/`, `tooling/external-verifier-hyperjump/`, `docs/external-verifier-conformance.md`) | `beta` | Two independent JSON-Schema implementations validate our published schemas. What actually watches for divergence is `tests/wat/test_external_verifier_parity.py`, driven by `scripts/external_verifier_validation.py`, in the ordinary test lane — the Node.js half skips when `node` or `node_modules` is absent, so it reports rather than blocks. Corrected 2026-09-14: this row previously credited `external-verifier-drift.yml`; that workflow never touched `tooling/` (it drove the four-pole verifier copy removed under ADR-0074) and no longer exists. |
