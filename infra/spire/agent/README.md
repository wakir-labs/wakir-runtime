# SPIRE-Agent-Sidecar Federation-Wiring
...
This directory carries the SPIRE-Agent-Sidecar
substrate wired into the Federation-Bundle-Endpoint.
Two SPIRE-Agent containers (one per trust-domain: `wakir.test` +
`partner.test`) attest against their side's SPIRE-Server, ingest the
federated peer-bundles via the bundle-endpoint listener, and
...
| `bin/spire-agent-fed-reload` | Bundle-Cache-Refresh Helper: cron-cycle + SIGHUP. |
...
| `tests/test_spire_agent_fed_reload.py` | Reload-helper: snapshot determinism, change-detection, cron-cycle step, SIGHUP-style trigger, race-condition recovery. |
...
```bash
# Pre-condition: federation substrate must already be up.
...
# Now bring up the agents alongside.
cd ../agent
...
# different cert_hint (selector-to-trust-domain mapping is the
# substrate invariant).
./bin/spire-agent-fed-attest fetch-x509 \
    --selector "1000:1000:/usr/bin/wirelang" \
...
With both federation servers and federation agents up, the
live X.509-SVID cross-trust verify is the acceptance
...
The `Federated Bundles` block in the output is the
...
the RealAdapter constant, so the end-to-end fallback contract is
...
This is the same recipe pattern as README §3b. The sandbox does
...
`Federated Bundles` line from §3b) as the acceptance
evidence.
...
Five test files, **77 tests, all green** (adds 14 reload tests):
...
The acceptance is **green hermetic test surface + this
...
### Hermetic test surface (invariants)
