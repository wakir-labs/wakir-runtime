...
Status: draft, Phase-2 (Z-B 7th-bucket
`wakir-capability-policies` paired-update — see §1, §3.2.3, §7.9
and the §8 stamp; added the
...
| 3.2.1 | Backfill 5th bucket on pre cluster                         |
| 3.2.2 | Backfill 6th bucket on pre cluster                         |
| 3.2.3 | Backfill 7th bucket on pre cluster                |
...
| --------------------------------- | ------- |
| `wakir-schemas`                   | 5       | unbounded  | 256 KiB   | file    | 1        | Wirelang schema-registry cache (Phase-1b consumer:)                                            |
...
| `wakir-federation-routes`         | 5       | unbounded  | 4 KiB     | file    | 1        | V-908 federation-route registry (Phase-1b consumer: Wirelang-side `NatsKvRouteRegistry`/) |
...
The 5th bucket `wakir-schema-registry-entries` was added
...
cluster). The Wirelang-side schema-registry backend
...
The 6th bucket `wakir-federation-routes` was added as
the Z-B follow-up paired-update, closing the Z-B
...
snapshot layer). Before the bucket was created out-of-band per
the §6.5 hand-creation recipe; promotes it into the routine
...
The 7th bucket `wakir-capability-policies` was added as
the Z-B paired-update with the Wirelang-side
...
JSON-file shape (publisher-CLI; see
...
cross-import-mirror anchor). The pre-Phase-2-reservation
pattern is the analogue; the 6th-bucket pattern is the
...
onward; pre clusters that used the §6.5
...
**Image-pin form (post-Phase-2, Cross-Review Zone-C):**
...
#    (Older clusters report four created + two "would
#    create" or "missing"; clusters that already
...
### 3.2.1 Backfill the 5th bucket on a pre cluster (Phase-2)

Clusters brought up before have only the four
Phase-1b-consumed buckets. After pulling the runtime onto the
...
### 3.2.2 Backfill the 6th bucket on a pre cluster (Phase-2)

Clusters brought up before either (a) used the §6.5
...
pulling the runtime onto the build host, an operator backfills
...
### 3.2.3 Backfill the 7th bucket on a pre cluster

Clusters brought up before have no
...
the runtime onto the build host, the no-downtime
backfill is:
...
the Wirelang-side `BUCKET_CONFIG` constant lands as a
Follow-up once the Wirelang-track-owned encoder/decoder
...
### 5.1 `check-nats-kv-health` tool (Phase-1b)
...
configuration drift the way `init-nats-buckets.py` does. ships
...
JSON output shape (stable for onwards; cron consumers may
rely on it):
...
forwarding) is out of scope for; the cron snippet is the
...
### 5.3 `check-federation-evaluator-health` tool (Phase-1b)
...
and `wirelang/federation/n2_evaluator.py` byte-precisely;
...
| Connectivity (`/jsz`)                  | HTTP probe (re-used from)                                      | 2xx healthy / unreachable / skipped                     |
...
virtualenv. The NATS-KV health-check has no wirelang-eng-side
...
### 6.5 Federation-routes bucket creation (now routine; fallback only) (Phase-1b, flip Phase-2)

The `wakir-federation-routes` bucket is the V-908 federation route
registry consumed by the N2 evaluator. As of **Phase-2**
...
pre clusters that used this hand-creation recipe are
