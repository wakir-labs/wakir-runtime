...
29 dead artefacts listed above were removed from the tree after the Phase-4
sweep (ADR-0072). They remain reachable from the tag `archive/pre-phase-4`,
which points at the pre-cleanup tip of every repository.

Four files were kept on purpose because live alert rules and the aggregator
monitoring guide still name them:
`persona-engine-phase-3c-welle-status.json`, `phase-3-marathon-slo-final.json`,
`phase-3c-cross-welle-coordination.json`, `sli-slo-phase-3-marathon.md`.
Removing them requires updating those references first.

Evidence and reports under `evidence/` and `reports/` are not pruning
candidates. They carry the audit trail, including the OpenTimestamps
attestation of the Phase-3 completion marker.
