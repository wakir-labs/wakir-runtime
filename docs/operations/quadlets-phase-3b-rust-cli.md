...
**mini wave (+ 7-binary 8-binary
9-binary 11-binary extensions)** · Operations-Reife ·
Single-PR-Bundle
**Status:** Operator-Hand live bring-up after Cross-Review Zone-C
digest resolution. Hermetic format-validation tests live in
`tests/infra/test_quadlets_phase_3b.py`. (Retry after
quota-hit; identical bytes-shape as the aborted draft. The
mini wave extends the inventory from 5 to 7 binaries in
lock-step with the Cosign-Policy update per ADR-0065 Phase-3c
Trigger-Gate 2+3; 7->8 adds svid-workload-identity;
8->9 adds bridge-audit-writer; 9->11 adds bridge-audit-replay
and migrate-version — closing protocol engineering's Phase-3a-Foundation 15-module
sweep at 15/15 on the Cosign-Policy side and 11/11 on this
carrier-image installer.)

> **cross-reference.** The Operator-Hand recipe
> (Installer-Sequence + Cosign-Verification + Rollback-Pfad pro
> Binary + Cross-wave-Coordination) lives in
> `docs/phase-3c/quadlet-cosign-15-binary-installer.md`. This
> document remains the canonical/24 living operator
> reference; the doc is the substrate-refresh recipe for
> the carrier-image 9->11 + Cosign-Policy 13->15 transition.

## 1. Scope

The Phase-3b Rust-default switches (/18/19/20/22/23 mini wave
...
|---|---|---|---|---|
| 1 | `wakir-persona-engine-recovery` | `wirelang-rust/crates/persona-engine-recovery` | `WAKIR_RECOVERY_BACKEND=rust` | PR #167 |
| 2 | `wakir-persona-engine-state-backing` | `wirelang-rust/crates/persona-engine-state-backing` | `WAKIR_STATE_BACKING_BACKEND=rust_{inmemory,natskv}` | PR #167 |
| 3 | `wakir-persona-engine-fsm` | `wirelang-rust/crates/persona-engine-fsm` | `WAKIR_FSM_BACKEND=rust` | PR #169 |
| 4 | `wakir-persona-engine-v907-verify` | `wirelang-rust/crates/persona-engine-v907-verify` | `WAKIR_V907_VERIFY_BACKEND=rust` | PR #171 |
| 5 | `wakir-persona-engine-bridge-diff` | `wirelang-rust/crates/persona-engine-bridge-diff` | `WAKIR_BRIDGE_DIFF_BACKEND=rust` | PR #175 |
| 6 | `wakir-persona-engine-subscribe-loop` | `wirelang-rust/crates/persona-engine-subscribe-loop` | `WAKIR_SUBSCRIBE_LOOP_BACKEND=rust` | PR #181 |
| 7 | `wakir-persona-engine-anchor-emitter` | `wirelang-rust/crates/persona-engine-anchor-emitter` | `WAKIR_ANCHOR_EMITTER_BACKEND=rust` | PR #184 |
| 8 | `wakir-persona-engine-svid-workload-identity` | `wirelang-rust/crates/persona-engine-svid-workload-identity` | `WAKIR_SVID_WORKLOAD_IDENTITY_BACKEND=rust` | PR #191 + |
| 9 | `wakir-persona-engine-bridge-audit-writer` | `wirelang-rust/crates/persona-engine-bridge-audit-replay` | `WAKIR_BRIDGE_AUDIT_WRITER_BACKEND=rust` | |
| 10 | `wakir-persona-engine-bridge-audit-replay` | `wirelang-rust/crates/persona-engine-bridge-audit-replay` | `WAKIR_BRIDGE_AUDIT_REPLAY_BACKEND=rust` | PR #246 |
| 11 | `wakir-persona-engine-migrate-version` | `wirelang-rust/crates/persona-engine-migrate-version` | `WAKIR_MIGRATE_VERSION_BACKEND=rust` | PR #250 |

The/24 Quadlet bundle delivers two unit files:
...
- **Zone C (Container-Image-Pipeline x OTS-Anchoring):** the
  carrier image is the same `wakir-persona-engine` image that the
  the persona Quadlet pins from the consensus bundle. No new image-
...
- **Cosign-Policy parity (closed):**
  `policies/cosign-policy-phase-3b.yaml` now inventories the same
  seven binaries this Quadlet installer iterates.
  PR #182 closed the `bridge-diff` gap (5-binary inventory); the
  This PR extends both substrates in lock-step
  to a 7-binary inventory, adding `subscribe-loop` (PR #181)
  and `anchor-emitter` (PR #184). The
...
| Installer exits 2 with `missing or non-exec` | Carrier-image drift — the digest you have does NOT ship all seven binaries. Halt rollout, escalate to Zone-C cross-review (was the image rebuilt without one of the/22/23 binaries?). |
| `/opt/wakir/bin/<binary> --version` segfaults on the host | Host glibc vs musl mismatch — the binaries are statically linked against musl per the persona-engine Containerfile.real; if this surfaces it indicates a build-substrate drift, not a Quadlet drift. Escalate to persona-engine engineering / Rust-build-owner. |
...
| **Rust-CLI installer Quadlet (/24)** | `quadlet/wakir-rust-cli.container` | Carrier image + 7 binaries host-side install | Operator-Hand resolve-image-pins-ci |
...
bring-up is Operator-Hand per ADR-0051 Sandbox-vs-Host-
Operations-Trennung.
