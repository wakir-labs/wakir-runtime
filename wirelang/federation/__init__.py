...
This module introduces the live ``peer_org`` /
...
This module adds the **multi-org federation substrate**
sub-module (``multi_org_substrate.py``) as a Phase-3-preparation
layer composing additively over the ``wakir-federation-routes``
bucket. The substrate is mock-only; live SPIRE bundle / did:web
resolution is a later increment.

This module adds the **multi-org-attestation NATS-KV
backend** sub-module
...
This module adds the **SPIFFE cross-trust-domain bridge**
sub-module (``spiffe_cross_trust_domain_bridge.py``) — the live
counterpart to the mock-bridge resolver. The bridge orchestrates
...
This module adds the **Capability-Attenuation-Chain-
...
the :class:`~wirelang.schemas.registered_by_capability.CapabilityPolicyRegistry`.

This module adds the **UnrevokeAuditMarker cross-org
...
:class:`UnrevokeAuditMarker` (internal audit-trail
...
This module adds the **multi-org-attestation live-tail
replicator** sub-module
(``multi_org_attestation_live_tail_replicator.py``) — the
continuous-stream composition layer on top of the ``bootstrap_multi_org_attestation_target_from_source`` one-shot
...
This module adds the Multi-Marker-Policy-Composition reducer:
