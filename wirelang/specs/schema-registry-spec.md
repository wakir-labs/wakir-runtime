...
**Pfad-B Phase-2c Live-HTTPS counterpart adapter (2026-05-15).** This
version adds one substantive load-bearing addition over v0.36.0: the
**`LiveHttpsSpireFedBundleFetcher` adapter**
(`wirelang.federation.spire_fed_bundle_live_https_fetcher`) — the
Phase-2c live HTTPS counterpart to the v0.36.0 hermetic
`SpireFedBundlePeerTrustBundleFetcher`. The new adapter implements
the same `PeerTrustBundleFetcher` Protocol by issuing an HTTPS GET
against a real SPIRE-Server federation bundle endpoint listener,
parses the response, and constructs a bridge-consumable
:class:`FetchedTrustBundle`. Closes the Open-Item on URL-shape:
SPIRE-Server bundle endpoint serves at the listener's **root path**
(`"/"`), empirically verified against the upstream Go implementation
(`pkg/server/endpoints/bundle/server.go`); the new live-fetcher's
pinned-URL convention therefore drops the `/bundle` suffix the
hermetic adapter used. Compatibility note: the hermetic adapter
remains byte-identical; both adapters now coexist as explicit
hermetic vs. live siblings.

**Pfad-B Multi-Org-Federation Live-Component adapter (2026-05-14).**
This version adds one substantive load-bearing addition over
v0.35.0: the **`SpireFedBundlePeerTrustBundleFetcher` adapter**
(`wirelang.federation.spire_fed_bundle_peer_fetcher`) that wires the
:class:`SpiffeCrossTrustDomainBridge` to the
`infra/spire/federation/bin/spire_fed_bundle` hermetic-mode CLI. The
adapter implements the bridge's :class:`PeerTrustBundleFetcher`
Protocol by exporting the hermetic-fixture JWKS for a pinned
(trust_domain → URL) mapping, returning a bridge-consumable
:class:`FetchedTrustBundle` value. Fail-closed at every step:
trust-domains not in the pin set raise
:class:`UnpinnedTrustDomainError`; bridge-supplied URLs that do not
match the pinned URL for the requested trust-domain raise
...
`wakir.federation.spire-fed-bundle-peer-fetcher/1`. Sandbox-only per
ADR-0051 — the adapter does NOT call podman, does NOT start a
...
changes; the bridge surface remains byte-identical; existing
single-org callers are unaffected. New §5.21 (Live- Component
peer-trust-bundle-fetcher adapter).

**Operator-CLI + Live-Tail-Replicator detect_replay-callsite-mirror
anchor (2026-05-13).** This version adds two substantive
load-bearing additions over v0.34.0: (1) the **operator-facing
marker-stack reducer CLI** `bin/wakir-marker-stack-reduce`
(`wirelang.cli.marker_stack_reduce`) wrapping the in-memory reducer
+ durable backend into one operator entrypoint with pretty-print
audit-trace rendering (deterministic marker-symbol legend
R/U/RI/CO/BR) and a stable JSON pipeline-mode for downstream tools;
(2) the **detect_replay-callsite-mirror** on the
`MultiOrgAttestationReplicator` — an additive optional
`replay_detector_fn` hook invoked after the filter and before the
apply step, plus a per-org `replay_drops_per_org` counter and a
`total_replay_drops` aggregate accessor. The default detector is
no-op pass-through; with the default the replicator's behaviour is
byte-identical to. v0.35.0 is an **additive minor bump** per §3.2;
no breaking changes; all pre-existing Replicator constructions
remain valid; the Tag-1 + surfaces remain byte-identical. New §11
(Operator- CLI annex) and §5.20 (Replicator-side
detect_replay-callsite-mirror).

**Durable Sequence-Number-Ledger + OTS-anchored Schema-Registry-Entry anchor
(2026-05-13).** This version adds two substantive load-bearing additions over
v0.33.0: (1) a **durable NATS-KV-backed `SequenceNumberLedger`**
(`wirelang.federation.sequence_number_ledger_kv`) that promotes the in-memory
ledger to production-grade multi-process / restart-safe persistence with
optimistic-concurrency CAS-pin and per-org bucket isolation; (2) an
**OTS-anchored schema-registry-entry** for
`wakir.federation.caveat-override-event-export/1`
(`wirelang/schemas/caveat-override-event-export.json`) with a pre-submission
anchor manifest fixture and Bitcoin-anchor pipeline contract. v0.34.0 is an
**additive minor bump** per §3.2; no breaking changes; the surfaces remain
byte-identical; the
:class:`~wirelang.federation.caveat_override_export.InMemorySequenceNumberLedger`
default is unchanged. New §5.18 (durable ledger) and §5.19 (OTS-anchored
schema-registry-entry).

**CaveatOverrideEvent Cross-Org-Export-Surface anchor
(2026-05-13).** This version adds the cross-org export surface
for the `CaveatOverrideEvent`
(`wirelang.federation.caveat_override_export`). Pattern-mirror on
the `UnrevokeAuditMarker` cross-org-export. New load-bearing
substantive additions over the pattern: (1) **replay-protection**
via per-(route_id, original-caveat-chain-hash) monotonic
sequence-numbers carried in the export envelope and validated on
a pluggable `SequenceNumberLedger`; (2) **bridge-verification** —
...
override_event_id carries witness-route bindings; (5)
**audit-trace projection** stripped of free-form reason text.
v0.33.0 is an **additive minor bump** per §3.2; no breaking
changes; all pre-existing frames remain valid against v0.33
verifiers; the Tag-3/surfaces remain byte-identical.

**v0.32.0 annex-note.** Version v0.32.0 is reserved for the
parallel Selin Phase-2 `persona-engine-format-spec.md` bump
(parallel-deliverable). The two spec axes are orthogonal
(persona-engine vs federation-export); v0.33.0 lives under the
additive-minor convention and explicitly does not back-merge
into v0.32. If the Selin v0.32.0 lands first, the
...
**..Persona-Engine-Format integration anchor (2026-05-13).** v0.32.0
adds §10 Persona-Engine-Format integration, documenting the new axis-C
`wakir-persona-v1` target format
(`wirelang/schemas/wakir-persona-v1.json`), the new axis-A input schema
`persona-claude-native` (`wirelang/schemas/persona-claude-native.json`),
and the byte-deterministic mapping crate `persona-engine-format`
(Crate-8). v0.32.0 is an **additive minor bump** per §3.2 versioning
policy; no breaking changes; no envelope changes; no bucket-shape
changes; the schema-registry surface itself remains byte-identical. The
companion spec is `wirelang/specs/persona-engine-format-spec.md`
**v1.2** (landed v1.0, bumped to v1.1 for the Aisha-HR
Counter-Vorschlag-1 `synthesis_default_exceptions` table, bumped to v1.2
for the §3.7 lifecycle protocols (`despawn_clean`, `recovery_drill`,
`migrate_version`) layered over the byte-unchanged §3.3 state-machine
envelope; the v0.32.0 schema-registry surface is byte-stable across all
three v1.x bumps). Cross-Review-Zone-K (WAT-bridge) **non-touched**
functionally (V-907 hash function unchanged; §5 of
persona-engine-format-spec). Cross-Review-Zone-J (container-bridge with
Kai) **TRIGGERED** as shape-only reservation (no Kai-side Quadlet change
yet).

**Persistent Marker-Stack NATS-KV Backend anchor (2026-05-13).**
This version adds the persistent storage tier for the
marker-composition engine
...
the in-memory `MarkerStack` for the reducer to return a
...
no breaking changes; the reducer surface remains byte-identical;
all pre-existing frames remain valid against v0.31 verifiers.

**Multi-Marker-Policy-Composition anchor (2026-05-13).**
v0.30.0 added the Multi-Marker-Policy-Composition engine
(`wirelang.federation.marker_composition`) on top of the
Cross-Org-Attenuation-Chain-Verifier (v0.29.0). The
composition engine collapses heterogeneous marker streams
...
v0.30 verifiers; the Cross-Org verifier surface remains
byte-identical.

Cross-reference: closeout source at
`agents-workspaces/reza/outbox/2026-05-13-sprint-7-wirelang-side-closeout-skizze.md`
§2 (six achievement-blocks with per-tag substance and push-SHAs), outbox rapport at
`agents-workspaces/reza/outbox/2026-05-13-reza-sprint-8-tag-2-verifier-hardening.md`
§A (Cross-Org verifier substance + tests), and outbox rapport at
...
The RealAdapter-Mirror (`wirelang/adapters/real_nats_adapter/`)
remains documented in `identity-substrate.md` §5.8 (paired with
`real_spiffe_workload_api.py` Slot-2-Mirror); the adapter is an
implementation surface, not a schema-registry surface — no
bucket-shape change, no envelope-shape change, no
...
the V-908 `RouteRegistry` Protocol shipped in.

The registry is the natural consumer of Kai's Phase-1 NATS-KV
inventory `wakir-schemas` bucket (`scripts/init-nats-buckets.py`).
This revision lands the production-target backend wrapper; Phase-1c
will land the schema-distribution flow that publishes
module-shipped schemas onto the bucket on operator command.
...
- The publisher CLI (`wirelang/schemas/publisher_cli.py`) exposing a
  ...
  modes over the Tag-1 + backends) and `dry-run` (validate
...
- The cross-bucket replication layer (`wirelang/schemas/replication.py`)
  ...
  composing source-side watch-stream with Tag-1 / Tag-3
...
- The replicator is a *thin composition*: it adds no new method on
  ...
  the existing Tag-1 + Tag-3 + surfaces.
...
- 12 hermetic determinism tests (T-SR-REP-01..12) over the same
  in-memory CAS-aware KV mock used by Tag-3 / Tag-4 /.
...
- ~~OI-7-Phase-1c-CAS~~ (CONSUMED in).
- ~~OI-7-Phase-1c-watch~~ (CONSUMED in).
- ~~OI-7-Phase-1c-publisher~~ (CONSUMED in).
- ~~OI-7-Phase-1c-replication~~ (CONSUMED in).

**Tag-1 (this revision, additive):**
...
- The signing primitive is **Ed25519 over SHA-256 of the
  ...
  same JSON envelope the codec emits, with the `signature`
...
- **VerifyMode** policy:
  ...
  - `STRICT` (Phase-2-end): entries without a `signature` slot
    ...
    is operator-controlled; This revision ships only the policy
    surface, not the activation switch.
- **`NatsKvSchemaRegistry` surface remains UNCHANGED** in Sprint-4
  . Signing happens at envelope-build time: caller signs the
...
- **Boundary**: this slot does NOT fetch the AIP document over
  ...
  (verification stays caller-driven, consistent with Tag-1
  Phase-2 boundary).
...
- **Transport delegation**: HTTPS fetch is delegated wholesale to the
  Phase-1b V-908 `HTTPSDocumentTransport` (PS-6 module); no
  ...
  invariants. The layer is therefore byte-thin over the existing
  transport.
...
- 12 hermetic determinism tests (T-AIP-FT-01..12) over the URL
  ...
  cross-layer composition with the `kid_resolver` plus a
  determinism / passthrough probe.
- This slot closes the §5.9 boundary item
...
- **Cross-Review-Zone-1 (Identity-Substrate) non-touched**: the four
  Z-1-K-consensus points remain byte-identical. The
...
- **Stateful tier on the stateless verifier**: the underlying
  `wirelang.identity.verify_aip_signature` is
...
- **Cache-key body digest is byte-equal to the Tag-4
  ...
  pay the JCS+SHA-256 cost once: the transport-fetch produces
  the byte-anchor; the cache key reuses the same byte-anchor
  shape. The two computations are byte-equal by construction (same
  helper layer); the module recomputes from the body for API
...
- This slot consumes the **AIP-document signature-verification
  ...
  Identity-Substrate slot in §5.3; This revision ships it now to
...
- **`NatsKvSchemaRegistry` surface remains UNCHANGED**. No backend
  ...
  layer alongside `verify_aip_signature` and the
  kid-resolver / transport-fetch.
- **Cross-Review-Zone-1 (Identity-Substrate) non-touched**: the four
  Z-1-K-consensus points remain byte-identical. The cache
  ...
  STRICT-mode policy (`VerifyMode.STRICT`) is
...
- **Boundary**: this slot does NOT replace
  ...
  `HTTPSAipResolverCache` and the transport-fetch
...
- **Additive authorisation on top of cryptographic verification.**
  ...
  NOT replace any signing-side surface. The signing primitive
  remains the single source of truth for signature correctness; the
  gate is an *additional* check that callers MUST run
...
- **`NatsKvSchemaRegistry` surface remains UNCHANGED**. No backend
  ...
  CLI-level slot, not substance).
- **Cross-Review-Zone-1 (Identity-Substrate) non-touched**: the
  four Z-1-K-consensus points remain byte-identical. The
...
The Phase-1c slots are all consumed: consumed **OI-7-Phase-1c-CAS**,
consumed **OI-7-Phase-1c-watch**, consumed
**OI-7-Phase-1c-publisher**, consumes **OI-7-Phase-1c-replication**.
consumes **OI-7-Phase-2-sig**; This revision closes the
Z-1-Sprint-4-Anhang follow-up `kid → public-key resolver`; This
revision closes the §5.9 boundary item `AIP-document
transport-fetch`; This revision ships the AIP-document
signature-verification cache tier (previously listed as a Phase-3
Identity-Substrate slot) so the Phase-2 verifier-pipeline
performance profile is rounded out. The Phase-2 reserved slots are
tracked as **OI-7-Phase-2-quorum / -deprecation / -ipfs /
-bidir-replication / -resume** plus the STRICT-toggle follow-up slot
noted above.
...
is paired Z-B-update; This revision does NOT add a 6th bucket. The
registry is a *consumer* of the existing inventory slot; the only
Kai-side acknowledgement requested is a paired-update note in the
bucket-inventory cross-reference for Tag-1's "`wakir-schemas` is
consumed by Wirelang schema-registry backend" documentation surface.
...
ADR-0052 patch lifts to v0.2.1):
...
| `federation` | `datalog-caveat` | `0.2.1` | `datalog-caveat.json` (ADR-0052 Class-P promotion of `caveat_hash`) |
...
class NatsKvSchemaRegistry:
    ...
    async def get_with_revision(# Phase-1c
        self, key: str
    ) -> Optional[tuple[SchemaRegistryEntry, int]]: ...
    async def put(self, entry: SchemaRegistryEntry) -> int: ...
    async def put_with_revision(# Phase-1c
        self, entry: SchemaRegistryEntry, expected_revision: int
    ) -> int: ...
    async def delete(self, key: str) -> None: ...
    async def list_keys(self) -> list[str]: ...
    async def snapshot(self) -> InMemorySchemaRegistry: ...
    async def watch(self) -> _SchemaWatchStreamHandle:... # Phase-1c
```

The watch-stream additions also expose:

```python
class WatchOp(enum.Enum): # Phase-1c
    PUT = "PUT"
    DELETE = "DELETE"
    PURGE = "PURGE"

@dataclass(frozen=True)
class WatchEvent: # Phase-1c
    op: WatchOp
    key: str
...
@dataclass
class LiveSchemaSnapshot: # Phase-1c
...
### 5.3 What Tag-1 + Tag-3 + Tag-4 + Tag-5 + Tag-6 + Tag-1 + Tag-3 + Tag-4 + Tag-5 + Tag-6 covers, and what Phase-2 still does NOT do
...
- Composition contract: source-side reads use watch-stream
  surfaces (`open_watch_stream`, `WatchEvent`, `LiveSchemaSnapshot`
  via `snapshot` for bootstrap); target-side writes use LWW
  (`put`) under `SOURCE_WINS` policy or CAS-pin
...
- The on-the-wire envelope schema is **additive only**: the existing
  ...
  **OPTIONAL** `signature` slot. No `/2` envelope is introduced; v0.5.0
  readers tolerate the new optional field (the backend codec is
...
- The `kid` references a `public_keys` entry on the registering
  ...
  public key is the caller's responsibility (does
...
- Verification policy is governed by `VerifyMode`:
  ...
  - `STRICT` (Phase-2-end): entries without a `signature` slot
    raise `SchemaRegistrySignatureError`. Phase-2-end activation
    is operator-controlled; This revision ships the policy
...
- The module is a pure composition over the Phase-1b V-908
  `HTTPSDocumentTransport` (PS-6) and the V-908 §3.4 DNS-anchor
...
- 12 hermetic determinism tests (T-AIP-FT-01..12) covering URL
  ...
  the `kid_resolver`, and a determinism / passthrough probe.
- This slot closes the §5.9 boundary item
  "AIP-document transport-fetch (`did:web` / `aip:web` HTTPS
...
- Cross-Review-Zone-1 (Identity-Substrate) **non-touched**: the four
  Z-1-K-consensus points remain byte-identical (the
  `anchor_required` toggle is an orthogonal hard-vs-soft
  policy, NOT the Z-1-K-Sprint-4-4 STRICT-mode signing toggle).

**v0.9.0 lands (additive):**

- The AIP-document signature-verification cache tier
  (`wirelang.identity.aip_signature_verification_cache`: stateful
  in-process bounded LRU+TTL cache around the Tag-1
...
- ~~No persistent capability-policy distribution (the Tag-6
  ...
  materialises a `CapabilityPolicyRegistry` directly
...
### 5.4 CAS-pin operational contract
...
### 5.5 Watch-stream operational contract

The watch-stream contract is the canonical incremental-view idiom for
long-running supervisors. It mirrors the V-908 watch-stream pattern shipped
in `wirelang/federation/route_registry_nats_kv_backend.py`.
...
4. For `PUT`: decode `value` bytes through `_envelope_to_entry`
   (re-runs the envelope codec gates; a poisoned envelope
...
### 5.6 Publisher CLI operational contract
...
applies is either an operator-input shape gate (so a malformed JSON
file is rejected at the file-read layer) or a routing decision into
the existing Tag-1 / backend gates.
...
5. Backend write gates (only on `publish`):
   - gates 1-3 (`schema_id ↔ $id`, body-hash match, key↔triple
     match) → VALIDATION_ERROR (exit 4).
   - CAS-pin call (only for `cas` and `create-only` modes) →
...
- The publisher CLI is a *write-side* surface; it does NOT consume
  ...
  watch surface. Operators who want post-publish observability
...
- **Target side (CAS-pin or LWW):** writes go through
...
- **Operator-input side:** the publisher CLI is the
...
### 5.8 Entry-signing operational contract

The entry-signing layer is the canonical Ed25519 signature substrate
for schema-registry entries. It is a thin composition over the
envelope codec and the existing `wirelang.identity.aip_signing` JCS
+ SHA-256 + Ed25519 primitive; it adds no new method on
...
A `SignedSchemaRegistryEntry` is the in-memory pair of a
...
`SchemaRegistryEntry` (parity with the codec).
...
- `STRICT`: an unsigned entry raises `SchemaRegistrySignatureError`
  ...
  operator-controlled (out of scope here).
...
3. **Optional-slot backward compatibility:** envelopes WITHOUT a
   `signature` slot round-trip through the codec unchanged.
...
- The signing primitive is byte-identical to
  ...
  consistency.
- The `kid` field is a free-form string in; binding
  it to an AIP-document `public_keys` entry is the kid → key
  resolver's responsibility (out of scope for this slot). Cross-
  Review-Memo to Tomás (WAT-side) is recorded in Tag-1
  outbox §2.

**boundary:**

- Tag-1 ships the signing primitive and verify-mode policy;
  it does NOT ship the kid → key resolver.
- Tag-1 does NOT plumb signature verification into the
  backend read path; verification is caller-driven.
- Tag-1 ships the `STRICT` mode policy surface but NOT
...
reference to "the identifier" and is reconciled here by this
byte-accuracy note. Future implementations MUST match on `kid` to
...
This composition is the recommended Phase-2 verifier pattern. test
T-KID-RES-03 exercises it end-to-end against the RFC-8032 Ed25519
test-vector seeds.

### 5.10 AIP-document transport-fetch operational contract
...
already obtained the AIP document, but does not specify how. This
revision ships that step as a pure composition of two existing
Phase-1b primitives — the V-908 HTTPS-transport
(`HTTPSDocumentTransport`) and the V-908 §3.4 DNS-anchor pattern —
extended in shape from the FTD document to the AIP document. The
module is `wirelang.identity.aip_document_transport_fetch`; it does
NOT mutate the schema-registry backend, does NOT add a method to
...
`HTTPSDocumentTransport` (PS-6 module). The layer:
...
The `anchor_required` toggle is **orthogonal to** the
...
- Validate the AIP document against the JSON Schema
  ...
  Phase-1a `aip_resolver`; This revision is shape-agnostic.
- Mutate the schema-registry backend surface. `NatsKvSchemaRegistry`
  is unchanged; no new method, no new envelope.
- Cache the result. This revision is a stateless pure composition. The V-908
...
- **Z-1-K-Sprint-4-1 (kid-Resolver-Shape)** non-touched; Sprint-4
  closed it. feeds the resolver, does not modify it.
...
test `T-AIP-FT-11` exercises steps 1-2 against the RFC-8032
Ed25519 test-vector seeds; the resolver feed-through to
`verify_entry_signature` is covered by test `T-KID-RES-03`. The
two tests together pin the end-to-end Phase-2 verifier pattern.

### 5.11 AIP-document signature-verification cache tier

The verification-cache tier is a stateful, bounded LRU+TTL cache on
top of the AIP-document signing primitive
(`wirelang.identity.verify_aip_signature`). The verifier is pure
and stateless; the §5.11 cache memoises its Boolean outcome to
short-circuit re-verification cost on repeated calls with the same
`(body, signature, public_key)` triple. The module is
...
1. `SHA-256(JCS(body without document_signature))` hex of the
   AIP-document body. Identical in shape to the Tag-4
...
- Mutate `wirelang.identity.sign_aip_document` or any signing-side
  surface. This revision is read-side only.
...
- **Z-1-K-Sprint-4-2 (JCS-Resolver-Lock)** non-touched; this module
  ...
  byte-equal to the `_jcs_anchor_hex` shape.
...
- **Z-1-K-Sprint-4-4 (STRICT-Mode-Activation-Owner)** non-touched;
  ...
  the verifier; the caller's STRICT-mode policy (Tag-1
...
# (this section). Repeated calls with byte-equal
...
test `T-AIP-SVC-02` pins the hit-on-second-verify byte-equality with
a fresh `verify_aip_signature` call; T-AIP-SVC-03 pins the TTL
invalidation through an injectable clock; T-AIP-SVC-04 pins the LRU
eviction by insertion-order; T-AIP-SVC-05 pins the negative-caching
round-trip; T-AIP-SVC-08 pins the structural-failure non-caching
contract.

### 5.12 `registered_by`-capability-gating operational contract

The capability-gating layer is the Layer-3 authorisation tier for
Phase-2 schema-registry signing. It binds the `registered_by` field
of a `SchemaRegistryEntry` to an in-memory policy bundle that
...
The gate is *additive* policy on top of the `verify_entry_signature`
cryptographic primitive: a cryptographically valid signature can
still be denied if the issuer lacks capability over the registered
triple. Cryptographic correctness remains the single source of truth
in `wirelang.schemas.entry_signing` and is UNCHANGED.
...
2. **`allowed_kids`** is a non-empty tuple of `kid` strings. The
   entry's `signature_block["kid"]` MUST be a member. This revision ships
...
4. **`not_before` / `not_after`** define a validity window for the
   ...
   (consistent with the `resolve_kid` `as_of`
   contract).
...
2. **Caller-supplied `kid`:** the gate consumes
   ...
   It does NOT call the `kid_resolver`; the kid →
...
- This module does NOT ship Biscuit binary token encode/decode +
  ...
  substantiation. This revision is a *prelude*: a simpler in-process policy
...
- ~~This module does NOT distribute or persist policies. The
  ...
  Phase-3 slot.~~ (**CONSUMED in Tag-2** —
  ...
  bucket; see §5.14. The in-process
  `CapabilityPolicyRegistry` remains the evaluation surface;
  This revision ships the source-of-truth substrate that
...
- This module does NOT replace
  ...
  signature correctness; the gate is *additional*
  authorisation policy.

**Cross-Review-Zone-1 (Identity-Substrate) non-touched:**

- Z-1-K-Sprint-4-1 (kid-Resolver-Shape) is consumed *upstream* by
  the caller; takes the `signature_block["kid"]` byte-equal
  as the caller supplied it and checks set-membership against
  `allowed_kids`. This revision does NOT import or call
  `wirelang.identity.kid_resolver`.
- Z-1-K-Sprint-4-2 (JCS-Resolver-Lock) is non-touched: This revision does
  not canonicalise anything; it is policy-evaluation only.
- Z-1-K-Sprint-4-3 (Curve-Choice = Ed25519) is reinforced
  indirectly: operates on the entry-signing layer which is
  Ed25519, but carries no curve choice of its own.
- Z-1-K-Sprint-4-4 (STRICT-Mode-Activation-Owner) is non-touched:
  This revision is *additional* policy and is orthogonal to the
  `VerifyMode` toggle (which gates the *cryptographic*
  signature requirement). A registry running `VerifyMode.STRICT`
  may also run capability-gating; the two checks compose
  independently.
...
test `T-RBC-02` pins the matching-policy allow with the
policy-reference roundtrip; T-RBC-04 pins the kid-not-allowed deny
and the multi-policy short-circuit; T-RBC-05 pins the layer and
name dimensions of triple-not-allowed separately; T-RBC-09 pins the
validity-window semantics (inclusive `not_before`, exclusive
...
#### 5.12.1 Revocation operational contract (additive and..5)

This revision adds an *explicit revocation* axis to
`CapabilityPolicy` that is distinct from the `disabled` soft
kill-switch. The split is deliberate:

- `disabled=True` is a **reversible operator-side state** ("this
  policy is paused for now; we may re-enable it later"). The
  spec wording is intentionally non-committal.
...
remains the single value schema; This revision adds two optional
fields:
...
The decoder treats missing keys as `None` so..5 envelopes (without
these keys) decode byte-equally to unrevoked policies. The encoder
always emits both keys (`null`-explicit when absent) so
on-the-wire payloads are deterministic.

**Gate-precedence amendment.** The fallback ordering
(`POLICY_DISABLED` → `KID_NOT_ALLOWED` → `TRIPLE_NOT_ALLOWED` →
...
2. Otherwise fall through to the disabled / kid /
...
pre-CAS `Gate 3` check (Gates 1 and 2 are the record-type and
pair-key-derivation gates; Gate 3 is the new revocation-monotonic
gate). The gate reads the live record at the key; if it carries
`revoked_at != None`, the incoming record MUST either preserve
that instant byte-equally or refuse:
...
**LWW non-enforcement.** The `put` (LWW) path does NOT enforce
revocation-monotonicity. This is consistent with the rationale:
LWW writes are operator-deliberate and the CAS-pin path is the
safety-invariant guard. An operator who deliberately wants to
un-revoke must use the LWW path AND accept the audit consequences
(the revocation event remains in the bucket history depth,
retrievable for the configured `history=5` window).

**Watch-stream surface (unchanged).** A revocation write appears as
one PUT event with `event.record.policy.revoked_at != None`.
Consumer-side filters can subscribe to revocation events
...
The four Z-1-K-consensus points (kid-resolver shape, JCS-resolver
lock, curve choice Ed25519, STRICT-mode activation owner) remain
byte-identical.
...
**Boundary: This revision does NOT ship (explicit).**

1. Publisher-CLI `--revoke` flag composing the CAS-pin revocation
   path — candidate; the API surface is reachable
...
2. Watch-stream consumer-side revocation-event filter helper — the
   ...
   revoked_at != None`); a dedicated helper is Tag-1+
   candidate.
3. Cross-bucket revocation replication (extend Tag-6
...
4. Full Biscuit v3 binary-token revocation-list interpretation —
   ...
   This revision ships *policy-level* revocation, not *token-level*
...
### 5.13 Publisher-CLI capability integration

lifts the capability gate into the publisher-CLI operator surface. The
integration is **additive**: the `sign_entry` primitive and the
`gate_signed_entry` composition helper are wired in between
`_build_entry` and the backend `put` / `put_with_revision` call. No
new validation gate runs on the backend side; the on-the-wire envelope
is byte-unchanged; a deny short-circuits before the backend connect so
the `wakir-schemas` bucket is never touched on a deny.
...
The matrix adds **one** code; pre-codes are byte-unchanged:
...
The receipt adds three optional fields; pre-fields are byte-unchanged.
Pre-callers that omit the capability flags receive a receipt whose new
fields are at their default-off values (`signed=false`, `kid=null`,
`gate_decision=null`), so consumers indexing by the legacy field set
continue to read byte-equal pre-existing fields and consumers indexing
by the new fields receive an unambiguous "feature-off" marker:
...
#### Pipeline ordering (canonical capability-gated publish)

The flow is the operator-CLI projection of the §5.12 Composition
Pattern, with three differences relative to the in-process
code-block:
...
1. The verifier (`verify_entry_signature`) is NOT invoked on the
   ...
   consumer-side concern (Tag-1 §5.8 + Tag-5 §5.11 caching
   tier).

2. The capability registry is loaded from disk every CLI invocation
   (no caching across runs). This is a deliberate Tag-1
...
3. The signature block is **not** written to the bucket. Sprint-5
   publishes the underlying `SchemaRegistryEntry` only; the
   signature is local authorisation glue. Pushing the signed
   envelope onto the bucket is the slot (CLI flag
...
- Does NOT push the signature block onto the `wakir-schemas` bucket.
  ...
  the Tag-1 / Tag-3 / Tag-4 / stack and lives outside
  the publisher CLI.
...
- ~~Does NOT distribute capability policies. The registry is loaded
  ...
  reservation per §5.12 / §7.~~ (**CONSUMED in Tag-2** —
  `wirelang.schemas.capability_policy_nats_kv_backend` ships the
  `wakir-capability-policies` bucket and the
  `NatsKvCapabilityPolicyBackend` surface; see §5.14. Tag-1
  publisher CLI is byte-unchanged: the `--capability-registry` flag
  still reads operator-local JSON. ~~A future `--capability-bucket`
  publisher-CLI flag that reads from the bucket is a
  candidate; This revision is the substrate, not
  the CLI integration.~~ — **CONSUMED in Tag-3**: the
...
- Does NOT modify `NatsKvSchemaRegistry`. No new method, no envelope
  ...
  surface is byte-identical to Tag-1 / Tag-3 /.

- Does NOT modify `entry_signing.sign_entry` or
  `gate_signed_entry`. Tag-1 + primitives are
  imported and called byte-equal.

- Does NOT alter pre-receipts beyond adding three optional
  fields at default-off values. The bare-publish path (no capability
  flags) emits a receipt with the same pre-field set plus
  three trailing `false` / `null` / `null` values.

#### Cross-Review-Zone-1 non-touched (all four Z-1-K-points)

- **Z-1-K-Sprint-4-1 (kid-Resolver-Shape):** Tag-1 does NOT
  import `kid_resolver`; the operator supplies `--kid` directly.
- **Z-1-K-Sprint-4-2 (JCS-Resolver-Lock):** Tag-1
  canonicalises nothing; signing-side canonicalisation lives inside
  `sign_entry` unchanged.
- **Z-1-K-Sprint-4-3 (Curve-Choice = Ed25519):** Tag-1
...
- **Z-1-K-Sprint-4-4 (STRICT-Mode-Activation-Owner):** Sprint-5
  This revision is orthogonal to `VerifyMode`; the publisher path does not
...
test `T-SR-PUB-CG-01` pins the loader round-trip; `T-SR-PUB-CG-02`
pins the `--sign` happy path receipt shape and signature-verify
round-trip; `T-SR-PUB-CG-03` pins the `--sign --gate` happy path
(POLICY_MATCH on a single-policy registry); `T-SR-PUB-CG-04..06`
pin the three deny axes (`no_policy_for_issuer`,
`kid_not_allowed`, `triple_not_allowed`) each as exit-code 7 with
the bucket left untouched; `T-SR-PUB-CG-07..08` pin the cross-flag
consistency rejections (orphan flags, `--gate` without `--sign`,
`--sign` without `--kid` or key source); `T-SR-PUB-CG-09` pins the
dry-run sign+gate path including the deny-on-dry-run case;
`T-SR-PUB-CG-10` pins the receipt-shape forward-compat
(default-off values for callers that omit the capability flags).

#### Bucket policy source (additive)

This revision closes the operator-experience gap left by §5.14: the
publisher CLI gains the `--capability-bucket` flag (mutually
exclusive with `--capability-registry`) so a sign+gate publish can
draw its policies from the persistent bucket
...
connect / cleanup contract from. A poisoned bucket envelope
(non-JSON value, schema-URI mismatch, malformed `allowed_triples`,
etc.) raises `CapabilityPolicyBackendError` which the CLI routes to
`ExitCode.VALIDATION_ERROR = 4`; the schema bucket remains untouched
in that case.
...
- No `--capability-bucket` Biscuit-v3-binary-token interpretation
  (the bucket envelope is the JSON shape; promotion
...
- No mutation of `NatsKvCapabilityPolicyBackend` (is
  consumed byte-unchanged; the layer reads from the existing
  `snapshot_registry()` surface).
- No new bucket on the orchestrator inventory (Z-B non-touched;
  `wakir-capability-policies` is the bucket consumed
  as-is).
...
The test inventory `T-SR-PUB-CB-01..10` (see §6.10 addendum) pins
the bucket-source happy path, the three deny axes through the bucket
source, the mutex enforcement, the orphan-flag rejection, the
poisoned-envelope route, the dry-run bucket path, and the
cross-source byte-equality contract. The auxiliary
`TestAuxBucketLoader` pins the factory-cleanup invariant and the
multi-policy sorted-key iteration order.

### 5.14 Capability-policy persistent distribution

This revision ships
`wirelang.schemas.capability_policy_nats_kv_backend`, the
persistent-distribution tier for the in-process
`CapabilityPolicyRegistry`. The module persists capability policies
on a dedicated NATS-JetStream-KV bucket (`wakir-capability-policies`)
so policy authorship survives operator-process restarts and so
multi-host deployments can distribute policies through the same
cluster substrate that already carries the schema registry
(`wakir-schemas`), AIP cache (`wakir-aip-cache`), FTD cache
(`wakir-ftd-cache`), FTD poison-marker (`wakir-ftd-poisoned`),
schema-registry-storage reservation
(`wakir-schema-registry-entries`), and federation routes
(`wakir-federation-routes`).

#### Public surface

The module exports (surface):
...
- `BUCKET_CONFIG` — the mirror constant for the orchestrator-side
  ...
  (Phase-2)"`. Drift-policy is identical to the Tag-1
...
- `CapabilityPolicyRecord` — a frozen dataclass wrapping a Sprint-4
  `CapabilityPolicy` plus operator-side bookkeeping
...
`registered_by` axis matches the `CapabilityPolicy` field byte-equal
(i.e. the same value that the schema-registry-entry carries in its
own `registered_by` field). The `policy_id` axis is
...
uses a 2-tuple because policies are issuer-bound, not triple-bound —
the triple-set lives inside the policy's `allowed_triples` field.
...
1. The record's embedded `CapabilityPolicy` bundle has already
   passed `CapabilityPolicy.__post_init__` (Tag-6
...
The envelope decoder round-trips through the `CapabilityPolicy`
constructor so every invariant is re-enforced at decode time.
Malformed envelopes surface as `CapabilityPolicyEnvelopeError` (the
persistent layer wraps the `RegisteredByCapabilityError` so
consumers can catch on the persistence boundary). A poisoned
(non-JSON, wrong schema, missing field, malformed `allowed_triples`
shape, non-boolean `disabled`,...) value raises
...
same bucket state yield byte-equal records and byte-equal sorted-key
lists (determinism analogue).
...
`add_policy`. The gate (`check_registered_by_capability`) consumes
the resulting registry byte-identical to the in-process path — the
only difference is the registry's origin (NATS-KV-backed vs.
operator-supplied at construction).

Disabled policies are included in the materialised registry; the
gate evaluates them (semantics: a disabled policy contributes a
fallback `POLICY_DISABLED` decision-source when no allowing match
was found). Operators evict a policy from the gate entirely by
calling `delete` and re-snapshotting.

#### boundary

- This module ships the persistent-distribution tier. It does NOT
  replace the in-process registry: operators who
  prefer the `--capability-registry` JSON-file path
...
- ~~This module does NOT auto-distribute policies to publisher-side
  ...
  (analogous to the schema-registry watch-stream).
  The slot is full-snapshot only.~~ (**CONSUMED
  in Tag-5** — the live tail
  ...
  . The full `snapshot_registry` path remains supported and
...
- ~~This module does NOT ship a CAS-pinned upsert path. Policies are
  ...
  ). This revision ships PUT (LWW) only.~~ (**CONSUMED
  in Tag-4** — the CAS-pin path `put_with_revision` is now
  the opt-in lost-update-protection surface, byte-mirroring the
  schema-registry CAS-pin contract; see the
  "CAS-pin operational contract" subsection below.
  The LWW `put` path remains supported and orthogonal
  to the CAS-pin path.)
- ~~This module does NOT modify the publisher CLI. A
  future `--capability-bucket` flag that reads policies from this
  bucket is a candidate; This revision is the
  ...
  end-to-end; see §5.13's "Bucket policy source"
  subsection. substrate is consumed byte-unchanged.)
...
- **Z-1-K-Sprint-4-1 (kid-Resolver-Shape):** Tag-2 does
  NOT import `kid_resolver`. The `allowed_kids` field on a
  persistent policy carries kid strings byte-equal to the in-process
  bundle; resolver chain is upstream of the gate
  call.
- **Z-1-K-Sprint-4-2 (JCS-Resolver-Lock):** Tag-2 does
  ...
  to `schema_body_sha256`). The entry-signing path
  and its JCS canonicalisation are byte-unchanged.
- **Z-1-K-Sprint-4-3 (Curve-Choice = Ed25519):** Tag-2 is
  curve-agnostic. The persistent policy carries `allowed_kids`
  strings without a curve-axis; the kid-resolver and
  entry-signing primitives remain the curve-binding
  layer.
- **Z-1-K-Sprint-4-4 (STRICT-Mode-Activation-Owner):** Sprint-5
  This revision is orthogonal to `VerifyMode`. The persistent-policy layer
  ships persistence, not signature verification.

#### Cross-Review-Zone-B paired-update (Kai-side action)

IS a Zone-B trigger. The orchestrator-side
`PHASE_1_BUCKETS` inventory currently lists 6 slots:

| Slot | Name                              | Status                                    |
|------|-----------------------------------|-------------------------------------------|
| 0 | `wakir-schemas` | Live (consumer) |
| 1    | `wakir-aip-cache`                 | Live (AIP resolver)                       |
| 2    | `wakir-ftd-cache`                 | Live (FTD resolver)                       |
| 3    | `wakir-ftd-poisoned`              | Live (FTD poison-marker)                  |
| 4 | `wakir-schema-registry-entries` | Phase-2 reservation |
| 5    | `wakir-federation-routes`         | Live (V-908 consumer)                     |

requests a 7th slot (`wakir-capability-policies`) via the paired-update memo to the DevOps track
(`agents-workspaces/kai/inbox/2026-05-11-reza-z-b-seventh-bucket-capability-policies-paired-update.md`).
The Wirelang-side `BUCKET_CONFIG` is the byte-anchor; the orchestrator-side `BucketSpec` will mirror it
byte-precisely on `history` / `ttl_seconds` / `max_value_size` / `storage` / `replicas` on Kai-side
acceptance (Kai-side slot). The Wirelang-side consumer is byte-functional once the bucket is
materialised on the live cluster (operator-hand `nats kv add wakir-capability-policies...` or routine
...
#### CAS-pin operational contract (additive)

This revision adds an opt-in lost-update-protection surface to the
persistent capability-policy backend. The substance is a
pattern-mirror on the schema-registry CAS-pin contract (see §5.4):
identical typed-exception shape, identical KV-adapter shim
contract, identical determinism invariants. The mirror is
intentional — operators who already learn the schema-registry
CAS-pin idiom (`get_with_revision` → `put_with_revision` → catch
`*ConflictError` → re-read on conflict) see the same idiom on the
capability-policy layer, with the only type-axis difference being
the conflict exception class (`SchemaRegistryConflictError` ↔
`CapabilityPolicyConflictError`).

**Why CAS-pin on capability-policy authorship.** The LWW `put` path
is acceptable when policy authorship is rare and
...
schema-registry CAS-pin contract:
...
- bucket configuration (`BUCKET_CONFIG`) is
  ...
  paired-update memo.
- The schema-registry CAS-pin surface
  ...
  capability-policy CAS-pin surface is a parallel
...
#### Watch-stream operational contract (additive)

This revision shipped the CAS-pin write path on the
`wakir-capability-policies` bucket. This revision adds the live-tail
*read* path, mirroring the schema-registry watch-stream contract
byte-precisely (with the predictable shift of class identities and noun
choices: `WatchOp` → `CapabilityPolicyWatchOp`, `WatchEvent` →
`CapabilityPolicyWatchEvent`, `LiveSchemaSnapshot` →
`LiveCapabilityPolicySnapshot`, `entry` → `record` on the event
payload).

**Why a watch-stream on capability-policies?** The full
...
watcher shapes (byte-equal to the schema-registry contract):
...
`NatsKvCapabilityPolicyBackend.snapshot`. This revision does NOT
silently swallow envelope poison (same contract as the full
`snapshot` path).
...
- A CAS-pin write (`put_with_revision`) yields one PUT
  event on the stream identical to a LWW PUT.
- A LWW write (`put`) yields one PUT event.
...
`LiveCapabilityPolicySnapshot._live` map is keyed by the full KV key
(`capability-policies/<registered_by>/<policy_id>`), not by
`registered_by` alone. This indirection is intentional: the
`CapabilityPolicyRegistry` is indexed by `registered_by` only (a
single issuer may carry multiple policies); the watch-stream needs
per-`(issuer, policy_id)` update / delete semantics so a DELETE
event on `capability-policies/<issuer>/<id-A>` does not nuke the
issuer's other policies. `as_registry` rebuilds the
`registered_by`-indexed view from the per-key map on each call.

#### Boundary: This revision does NOT ship (explicit)

- Watch-stream resumption / replay-from-revision
  (`watchall(..., resume_from=...)`) — Phase-3 slot. nats-py
  supports it; the wrapper exposes the underlying
...
- Publisher-CLI composition of the live-tail consumer (a
  ...
  `wakir-publisher`) — candidate. The Sprint-5
  `--capability-bucket` flag remains a one-shot snapshot read
  (no watch-stream tail). Operators who want the live tail use the
  Python API directly (or a daemon).
...
- ~~A `Replicator`-style cross-bucket capability-policy replication
  (analogous to schema-registry replication) —
  Phase-3 slot. This revision lands the watch-stream substrate; the
  replication-tier composition is reserved.~~ **CONSUMED** in
  v0.19.0: see
  `wirelang/schemas/capability_policy_replication.py` and the
  v0.19.0 change-log entry. The layer carries the
  revocation-monotonic invariant byte-precisely
  across the cross-bucket boundary (a feature with no
  schema-registry-side analogue).
- A Biscuit v3 binary-token interpretation of the policy envelope —
  Phase-3 substrate; the in-bucket JSON envelope shape is
  Tag-2 / byte-unchanged.
- A mutation of `NatsKvSchemaRegistry` — the schema-registry
  backend is byte-unchanged.

### 5.15 Multi-Marker-Policy-Composition

**Status:** Tag-3 lands the deterministic Multi-Marker-Policy-
Composition reducer in
`wirelang/federation/marker_composition.py`. fixes the
per-capability-token lifecycle reduction surface and the
closed-enumeration verdict surface; the OTS-anchored
...
- **`RevokeEvent(event_at, revocation_reason, tie_break, wat_anchor_manifest_id)`**:
  mirrors `policy.revoked_at` set; transitions
...
- **`UnrevokeEvent(event_at, previous_revoked_at, unrevoke_reason, tie_break, wat_anchor_manifest_id)`**:
  mirrors `UnrevokeAuditMarker` on the composition
...
- **`BridgeRevokedEvent(marker: BridgeRevocationMarker, tie_break, wat_anchor_manifest_id)`**:
  wraps the `BridgeRevocationMarker`. Bridge
...
- **Zone 1 (Identity-Substrate):** `BridgeRevocationMarker` is
  ...
  ). The composition module does NOT duplicate the schema.
...
- **Zone 3 (OTS-Schema-Anker):** reserved for a future Tag-N
  ...
  envelope). Not opened in.

#### 5.15.6 What explicitly does NOT do

- **No persistent marker-stack bucket.** The reducer is a pure
  in-memory engine; persistence is Tag-N+ candidate
  (pattern-mirror on capability-policy backend).
  **CONSUMED in Tag-4** — see §5.16.
- **No marker-stack cross-org-export-surface.** Mirror on
  `UnrevokeAuditMarker` export is Tag-N+
  candidate.
- **No operator-CLI surface.** A `reduce-marker-stack` CLI
  subcommand pretty-printing the audit-trace is Tag-N+
  candidate.
...
### 5.16 Persistent Marker-Stack NATS-KV Backend

**Status:** Tag-4 lands the persistent storage tier for the
marker-composition engine in
`wirelang/federation/marker_stack_kv.py`. Per-org append-only
NATS-KV buckets persist individual marker events; the live
`MarkerStack` is re-constituted on demand for the reducer to
yield a byte-identical `CompositionVerdict`. fixes the
on-the-wire envelope, the bucket-name family, the per-org
isolation contract, and the watch-stream filter contract; the
OTS-anchored schema-registry entry for the marker-event envelope
remains **reserved for a future tag** (Cross-Review-Zone-3
trigger).

#### 5.16.1 Why event-sourced (not single-blob)

The reducer is sort-stable: out-of-order arrivals collapse to
the same verdict. Persisting individual events rather than
overwriting a single `MarkerStack` blob is therefore a
natural fit:
...
2. `get_marker_stack` reads all events for the token-id and
   hands them to the `reduce_marker_stack`
...
PURGE on the entire token-id key-space (out of scope here) and
re-bootstrap.
...
input matching the `peer_org` permitted-character regex
...
`replicas=1`. Drift-policy: identical to the contract —
operator-managed, never auto-corrected.
...
Cross-org reads MUST flow through the explicit
...
- Decode failures raise `MarkerStackEnvelopeError` and
  terminate the iterator (consistent with the Tag-5
...
- **Zone O (Cross-Org-Federation):** non-touched. The
...
#### 5.16.10 What explicitly does NOT do

- **No cross-org-export-surface.** Cross-org export of
  marker-stacks (mirror on `UnrevokeAuditMarker`
  export) is candidate.
...
- **No mutation of the reducer.** The reducer
...
sandbox-vs-host separation policy). All tests run against an
in-memory mock that mirrors the `_MockKv` shape with an
additional `create` contract for append-only semantics. The
mock's `watchall` replays the bucket's current state to the
watcher before the live tail (byte-equal to the nats-py
`watchall` contract).

### 5.17 CaveatOverrideEvent Cross-Org-Export-Surface

This revision ships the cross-org export surface for the
`CaveatOverrideEvent` so verifier-observed issuer-side
caveat-narrowing events propagate across the
multi-org-federation substrate without leaking
operator-supplied free-form text or the raw caveat-chains
themselves. Pattern-mirror on `UnrevokeAuditMarker` cross-org
export with substantive new load-bearing gates:
replay-protection via monotonic sequence-numbers and a
pluggable durable ledger, structural narrowing classifier,
cross-org witness tuple, and a verifier-side symmetric
replay-detection function `detect_replay`.
...
The exporter applies the same pattern as Tag-5
...
ledger. This revision ships only `InMemorySequenceNumberLedger`
(hermetic-test-grade and single-process-exporter-grade); a
durable NATS-KV-backed implementation lives in Tag-N+
...
- **No durable ledger.** `InMemorySequenceNumberLedger` is
  ...
  NATS-KV-backed implementation is Tag-N+ slot.
- **No OTS-anchored schema-registry entry** for the
...
- **No mutation of the `CaveatOverrideEvent`
...
- **No mutation of the `UnrevokeAuditMarker`
...
- **No live NATS interaction.** Live NATS connections are
  ...
  policy). All tests are hermetic.
- **No replay-protection on the multi-org-attestation
  live-tail replicator**. The replicator
...
Live NATS connections are operator-hand. All tests are
hermetic and use the in-memory ledger; no FS interaction;
no network interaction.

### 5.18 Durable Sequence-Number-Ledger Backend

This revision ships the durable tier for the
...
| `SyncSequenceNumberLedgerAdapter` | class | Sync adapter implementing the `SequenceNumberLedger` Protocol |
...
permitted-character regex mirrors the `peer_org` predicate
alphabet (URI-safe ASCII subset, no slashes, no whitespace).
Two-way round-trip: `bucket_name_for_org` and
...
they are reproducible byte-for-byte per caller (Tag-2 / envelope
contract).
...
This is byte-equivalent to the `InMemorySequenceNumberLedger`
...
Cross-org reads MUST go through the explicit cross-trust-domain
bridge surface (`SpiffeCrossTrustDomainBridge` /
...
`SyncSequenceNumberLedgerAdapter` implements the
...
directly with `await`. Provided so callsites can swap their
default `InMemorySequenceNumberLedger` for the durable tier
without changing the exporter API.

#### 5.18.8 Was Tag-2 §5.18 explicitly does NOT do
...
- **No bridge cross-org admission.** A verifier in Org-B that
  ...
  here. The bridge surface handles cross-trust-domain
  semantics.
...
- **No multi-org-attestation live-tail-replicator integration.**
  The replicator currently has no per-payload
  replay-protection; a `detect_replay`-callsite-mirror in the
  replicator is a Tag-N+ candidate.

#### 5.18.9 Sandbox boundary

Live NATS connections are operator-hand. All durable-ledger
tests are hermetic and run against an in-memory mock that
mirrors the `_MockKv` shape with an `update` CAS-pin
extension. No live NATS connection is established from the
sandbox.

### 5.19 OTS-anchored Schema-Registry-Entry

This revision ships the OTS-anchored schema-registry-entry for
`wakir.federation.caveat-override-event-export/1`
(`wirelang/schemas/caveat-override-event-export.json`).
Pattern-mirror on the WAT-Layer-4 OTS-anchor pipeline
...
Bitcoin-anchor submission is **operator-hand**: the
sandbox-default `anchor_state` is `PRE_SUBMISSION`. The schema
digest is pinned (the manifest's `schema_file_sha256` must match
`sha256(schema-file-bytes)` of the live schema; tests enforce
this byte-for-byte) and the OTS-anchor-pipeline manifest is
materialised. The operator-side `anchor_root` submission step
materialises `schema.sha256.bin.ots` and flips the manifest to
`PENDING`. No live OTS-calendar or Bitcoin call is made from the
sandbox.

#### 5.19.5 Was Tag-2 §5.19 explicitly does NOT do
...
### 5.20 Replicator-side detect_replay-callsite-mirror (Teil B)

Teil B extends the `MultiOrgAttestationReplicator` with an
**optional replay-detector hook** invoked AFTER the filter and
BEFORE the apply step. The extension is additive: pre-existing
replicator constructions keep working byte-identical (default
detector is no-op pass-through).
...
A production detector that wires the `detect_replay` symmetric
gate over a downstream `SequenceNumberLedger` (durable variant)
is a thin adapter:
...
`MultiOrgAttestationWatchEvent` into a
`ExportedCaveatOverrideEvent` when the cross-org channel carries
override-event payloads. Teil B does NOT ship the extractor (the
production extractor lives downstream when the cross-org channel
design lands the override-event payload on the replicator's
stream); the hook itself is the attachment point.

#### 5.20.6 Sandbox boundary

Live NATS replication is operator-hand. All Teil B
detect_replay-mirror tests are hermetic and run against the
`_MockKv` shape extended with the mock watcher surface. No live
NATS connection is established from the sandbox.

### 5.21 Multi-Org-Federation Live-Component peer-trust-bundle-fetcher adapter (Pfad-B)

Pfad-B Tag-5 lands the **live-component** adapter that wires the
:class:`SpiffeCrossTrustDomainBridge` to the
...
- bridge is consumed via the *existing*
  `PeerTrustBundleFetcher` Protocol — no Protocol change.
-..modules are byte-identical to v0.35.0.
...
### 5.22 Multi-Org-Federation Phase-2c Live-HTTPS counterpart adapter (Pfad-B)

Pfad-B Tag-6 lands the **Phase-2c live HTTPS counterpart** to the
hermetic :class:`SpireFedBundlePeerTrustBundleFetcher`. The live
adapter implements the same :class:`PeerTrustBundleFetcher`
Protocol so a Wakir-side bridge can swap one for the other via a
single constructor call. The hermetic-vs-live choice is an
*operator concern*; the bridge layer is invariant.
...
1. Reject if `trust_domain` is not in the pin set
   (`UnpinnedTrustDomainError`, re-used from).
2. Reject if `url` does not equal the pinned URL for the requested
...
This decision closes the **Open-Item** (see
`2026-05-14-reza-sprint-7-pfad-b-tag-5-federation-live-component.md`
cross-review Zone-A D-2 Kai). The hermetic adapter's pinned-URL
shape included a `/bundle` suffix because the hermetic exporter is
opaque to URL — only the trust-domain literal feeds the JWKS
fixture. For the live fetcher the URL is **load-bearing** and MUST
be root-path. The hermetic suffix convention is therefore a
*hermetic-mode convenience* and does NOT constrain the live-mode pin
set.
...
the Phase-2 federation substrate. The hermetic module remains
hermetic-only; *this* module is the explicit import-graph signal
of the Sandbox-Host trennung
...
The live fetcher slots into the same Multi-Org-Federation
substrate as the hermetic adapter, exposing the identical
`PeerTrustBundleFetcher` contract:
...
-..module surfaces are byte-identical to
  v0.36.0.
- The live adapter is opt-in: callers MUST construct it
  explicitly. The hermetic adapter remains the recommended
...
This revision ships hermetic tests at
...
### 6.1 CAS-pin tests (additive)

This revision ships hermetic CAS-pin tests at
...
Total test additions: 10 primary CAS-pin tests + 2 auxiliary
probes = 12.

### 6.2 Watch-stream tests (additive)

This revision ships hermetic watch-stream tests at
...
- **T-SR-WS-05:** A frozen `InMemorySchemaRegistry` returned from
  ...
  the call. (Determinism contract, the invariant for verifier
  passes.)
...
Total test additions: 10 primary watch-stream tests + 2
auxiliary probes = 12.

### 6.3 Publisher CLI tests (additive)

This revision ships hermetic publisher-CLI tests at
...
Total test additions: 12 hermetic determinism tests
...
### 6.4 Replication tests (additive)

This revision ships hermetic replication tests at
...
used by Tag-3 / Tag-4 /, with a wrapper that simulates the
read-then-mutate race window for the CAS-conflict path.

### 6.5 Entry-signing tests (additive)

This revision ships hermetic entry-signing tests at
...
- **T-SR-SIG-06:** `envelope_with_signature` emits the envelope
  ...
  slot is the only difference versus the codec.
...
- **T-SR-SIG-12:** codec parity — a signed envelope emitted by
  `envelope_with_signature` is byte-equal to the corresponding
  unsigned envelope EXCEPT for the one optional `signature`
  slot. Stripping the slot from the signed envelope and re-decoding
  through the codec recovers the original entry byte-equal
  (the codec is unchanged in).

Total test additions: 12 hermetic determinism tests
(T-SR-SIG-01..12). The signing layer is pure: no NATS connections,
no I/O. Tests use `cryptography.hazmat.primitives.
...
### 6.6 kid-resolver tests (additive)

This revision ships hermetic resolver tests at
...
Total test additions: 12 hermetic determinism tests
(T-KID-RES-01..12). The resolver is pure: no transport, no signature
verification on the AIP document, no I/O. Tests use RFC 8032 Ed25519
test-vector seeds and hand-crafted minimal AIP-document fragments.

**Suite-level effect:** the wirelang test suite grows from 671
passed to **683 passed** (+12 net). The entry-signing tests
(T-SR-SIG-01..12) remain unchanged and green.

### 6.7 AIP-document transport-fetch tests (additive)

This revision ships hermetic transport-fetch tests at
...
- **T-AIP-FT-05** — non-object root body. A JSON array (`[1,2,3]`)
  ...
  enforces "JSON object root"; the defensive guard is dead
...
- **T-AIP-FT-11** — cross-layer composition. End-to-end:
  ...
  Pairs with `T-KID-RES-03` (the resolver →
...
Total test additions: 12 hermetic determinism tests (T-AIP-FT-01..12).
The fetcher is pure-composition: no signature verification, no schema
validation, no caching. Tests use a fake `urlopen` script + a stub
`TxtResolver` for end-to-end determinism; RFC 8032 Ed25519 test-vector
seeds are reused from the kid-resolver tests for the cross-layer
composition probe.

**Suite-level effect:** the wirelang test suite grows from **683
passed** to **695 passed** (+12 net). The entry-signing tests
(T-SR-SIG-01..12) and the kid-resolver tests (T-KID-RES-01..12) remain
unchanged and green.

### 6.8 AIP-document signature-verification cache tier tests (additive)

This revision ships hermetic verification-cache tests at
`wirelang/tests/test_aip_signature_verification_cache.py`. Inventory
T-AIP-SVC-01..12. All tests are hermetic: no real time (an
injectable `_FakeClock` provides deterministic monotonic ticks), no
I/O, no transport, no NATS. The Ed25519 signing primitive runs
in-process via the existing `cryptography` dependency; RFC 8032
test-vector seeds 1 and 2 are reused from the Tag-3 / test fixtures.
...
Total test additions: 12 hermetic determinism tests
(T-AIP-SVC-01..12). The cache is a pure performance optimisation: no
signature verification logic, no schema validation, no
transport-fetch coupling. Tests use an injectable `_FakeClock` for
hermetic TTL determinism; RFC 8032 Ed25519 test-vector seeds are
reused from the Tag-3 / fixtures.

**Suite-level effect:** the wirelang test suite grows from **695
passed** to **707 passed** (+12 net). The entry-signing tests
(T-SR-SIG-01..12), the kid-resolver tests (T-KID-RES-01..12) and the
transport-fetch tests (T-AIP-FT-01..12) remain unchanged and green.

### 6.9 `registered_by`-capability-gating tests (additive)

This revision ships hermetic capability-gating tests at
...
Total test additions: 12 hermetic test classes (T-RBC-01..12); 45
individual test functions when counting the parametrised
bad-arg-grid and per-scenario sub-tests. The gate is a
pure-function authorisation tier: no signature verification logic,
no schema validation, no transport-fetch coupling, no clock
dependency outside the explicit `as_of` argument.

**Suite-level effect:** the wirelang test suite grows from **707
passed** to **752 passed** (+45 net). The entry-signing tests
(T-SR-SIG-01..12), the kid-resolver tests (T-KID-RES-01..12), the
transport-fetch tests (T-AIP-FT-01..12), and the verification-cache
tests (T-AIP-SVC-01..12) remain unchanged and green.

### 6.10 Publisher-CLI capability integration tests (additive)

This revision ships hermetic tests at
`wirelang/tests/test_publisher_cli_capability_integration.py`. The
inventory is **T-SR-PUB-CG-01..10** plus an auxiliary loader-helper
coverage class. Tests are hermetic: no NATS, no real transport, no
real DNS, no wall-clock dependency for receipt determinism
(`--registered-at` is supplied; the Ed25519 seed is the RFC 8032
test-vector 1 32-byte seed).
...
- **T-SR-PUB-CG-10:** receipt-shape forward-compat — bare publish
  ...
  `gate_decision=null`; all pre-fields remain present;
...
**Suite-level effect:** the wirelang test suite grows from **752
passed** to **780 passed, 1 skipped, 7 subtests passed** (+28 net
through the T-SR-PUB-CG-01..10 family and auxiliary coverage).
The publisher-CLI inventory (T-SR-PUB-01..12) and the
capability-gating inventory (T-RBC-01..12) remain unchanged and
green; the tests are additive and exercise a parallel test
module.

#### Bucket policy source tests (additive)
...
**Suite-level effect:** the wirelang test suite grows from **804
passed, 1 skipped, 7 subtests passed** to **818 passed, 1 skipped, 7
subtests passed** (+14 net through the T-SR-PUB-CB-01..10 family and
the auxiliary class). The publisher-CLI inventory (T-SR-PUB-01..12),
the capability-gating inventory (T-RBC-01..12), the inventory
(T-SR-PUB-CG-01..10), and the inventory (T-CPP-01..10) remain
unchanged and green; the tests are additive and exercise a parallel
test module.

### 6.11 Capability-policy persistent distribution tests (additive)

This revision ships hermetic tests at
`wirelang/tests/test_capability_policy_nats_kv_backend.py`. The
inventory mirrors the `test_schema_registry_nats_kv_backend.py`
structure (T-CPP-01..10 + auxiliary probes):
...
- **`TestAuxEnvelopeShape`** (auxiliary probes): missing required
  ...
  empty `allowed_kids` rejected via the Tag-6
...
**Suite-level effect:** the wirelang test suite grows from **780
passed** to **804 passed, 1 skipped, 7 subtests passed** (+24 net
through the T-CPP-01..10 family and the two auxiliary classes).
The schema-registry-backend inventory (T-SR-01..10 + 2 aux), the
capability-gating inventory (T-RBC-01..12), and the
publisher-CLI-capability-integration inventory (T-SR-PUB-CG-01..10
+ aux) all remain unchanged and green; the tests are additive and
exercise a parallel test module.

### 6.12 Capability-policy CAS-pin tests (additive)

This revision ships hermetic tests at
`wirelang/tests/test_capability_policy_cas_pin.py`. The inventory is
a **pattern-mirror** on the `test_schema_registry_cas_pin.py`
structure (T-SR-CAS-01..10 + 2 auxiliary probes). The mirror is
byte-precise on test-shape; the only differences are the substrate
module under test (`capability_policy_nats_kv_backend` vs.
`registry_nats_kv_backend`) and the typed exception class
...
- **T-CPP-CAS-06:** gate-2 (pair ↔ key derivation) runs BEFORE the
  CAS call. The `CapabilityPolicyRecord`
...
- **T-CPP-CAS-09:** CAS+LWW orthogonality: a non-CAS `put` after a
  successful `put_with_revision` lands as LWW. The LWW path
...
**Suite-level effect:** the wirelang test suite grows from **818
passed, 1 skipped, 7 subtests passed** to **830 passed, 1
skipped, 7 subtests passed** (+12 net through the
T-CPP-CAS-01..10 family and the two auxiliary probes). The
schema-registry CAS-pin inventory (T-SR-CAS-01..10 + 2 aux), the
capability-policy LWW inventory (T-CPP-01..10 + 2 aux), and the
publisher-CLI bucket-source inventory (T-SR-PUB-CB-01..10 + aux)
all remain unchanged and green; the tests are additive and
exercise a parallel test module.

### 6.13 Capability-policy watch-stream tests (additive)
...
on the schema-registry watch-stream test inventory at
...
- **T-CPP-WS-09:** a frozen registry from a watch-fed snapshot gates
  identically to the full-snapshot path against the Tag-6
...
brings the count to **842** (+12 net). The schema-registry
watch-stream inventory (T-SR-WS-01..10 + 2 aux), the
capability-policy LWW inventory (T-CPP-01..10 + 2 aux), the
publisher-CLI bucket-source inventory (T-SR-PUB-CB-01..10 + aux),
and the capability-policy CAS-pin inventory (T-CPP-CAS-01..10 + 2
aux) all remain unchanged and green; the tests are additive and
exercise a parallel test module.

### 6.14 Capability-policy revocation tests (additive)
...
- **T-CPP-REV-05:** the gate denies with `POLICY_REVOKED` even when
  ...
  (window semantics): the validity-window check
...
- **T-CPP-REV-08:** an older..5 envelope (omitting
  the two new keys entirely) decodes via the decoder
...
- **T-CPP-REV-aux-lww-allows-unrevoke:** the LWW
  ...
  CAS-pin path, consistent with the rationale.

**Suite-level effect.** `wirelang/tests/` was 842 passing before;
brings the count to **854** (+12 net). The
`registered_by`-capability-gating inventory (T-RBC-01..12), the
capability-policy LWW inventory (T-CPP-01..10 + 2 aux), the
publisher-CLI bucket-source inventory (T-SR-PUB-CB-01..10 + aux),
the CAS-pin inventory (T-CPP-CAS-01..10 + 2 aux) and the
watch-stream inventory (T-CPP-WS-01..10 + 2 aux) all remain
unchanged and green; the tests are additive and exercise a parallel
test module.
...
- V-908 conflict-error pattern source:
  `RouteRegistryConflictError` in the same module (mirror).
- V-908 watch-stream pattern source: `WatchOp` / `WatchEvent` /
...
- **Phase-2 entry-signing: OI-7-Phase-2-sig — CONSUMED in Sprint-4
  ...
  touch).
- **Phase-2 kid → public-key resolver: CONSUMED in.**
  ...
  Identity-Substrate consumer of signature blocks:
...
- **Phase-2 AIP-document transport-fetch: CONSUMED in Sprint-4
  .** Module: `wirelang/identity/aip_document_transport_fetch.py`.
  Tests: `wirelang/tests/test_aip_document_transport_fetch.py`
  (T-AIP-FT-01..12). Closes the §5.9 boundary item
  "AIP-document transport-fetch (`did:web` / `aip:web` HTTPS bridge)".
  ...
  the kid-resolver (§5.9), and the schema-registry backend
...
- **Phase-2 AIP-document signature-verification cache tier: CONSUMED
  ...
  Substrate slot in §5.3; This revision ships it now to round out
  ...
  `jcs_sha256_hex` byte-anchor (Z-1-K-Sprint-4-2
...
- **Phase-2 `registered_by`-capability-gating: CONSUMED in Sprint-4
  ...
  Phase-2 slot in §5.8 boundary block (noted that
  capability-token gating on `registered_by` was reserved); Sprint-4
  This revision ships the in-process policy-bundle + gating-function tier
  ...
  top of the `verify_entry_signature` cryptographic
...
- Phase-2 Biscuit binary token encode/decode + Datalog evaluation:
  reserved (Phase-3 slot; the gate is the in-process
...
- ~~Phase-2 capability-policy distribution (persisted NATS-KV
  ...
  (Phase-3 slot; the registry is in-process only).~~
  (**CONSUMED in Tag-2** by
  `wirelang.schemas.capability_policy_nats_kv_backend`; see §5.14.
  The module ships the persistent-distribution tier
  for capability policies on the `wakir-capability-policies` bucket;
  the in-process `CapabilityPolicyRegistry` remains
  ...
  upserts (LWW-only in; Phase-3 slot) and watch-stream
  tail (full-snapshot only in; Phase-3 slot) remain
  follow-up slots. ~~Publisher-CLI `--capability-bucket` integration
  (candidate)~~ **CONSUMED in Tag-3** via
  ...
  see §5.13's "Bucket policy source" subsection and
  §6.10's "Bucket policy source tests" addendum.)
- ~~Phase-2 publisher-CLI integration of the gate:
  reserved (`wakir-schema-registry publish` flag that runs
  `gate_signed_entry` between `sign_entry` and `put`).~~
  **CONSUMED in Tag-1** (`wirelang/schemas/publisher_cli.py`
...
- **Phase-2 capability-policy CAS-pin: CONSUMED in.**
  ...
  The LWW `put` path is byte-unchanged and remains
  ...
  slot~~ (**CONSUMED in Tag-5** — see §5.14 Watch-stream
  ...
  watch-stream is the live-tail consumer substrate, not a
  CAS-quorum promotion. candidate: publisher-CLI
  composition of the live-tail consumer (a `--capability-bucket-watch`
  flag or daemon-mode subcommand) — explicitly NOT.
...
- **Phase-2 capability-policy explicit revocation: CONSUMED in
  ...
  Phase-3 slot. candidates: publisher-CLI `--revoke`
  flag composing the CAS-pin revocation path (CONSUMED in Sprint-6
  , v0.17.0); watch-stream consumer-side revocation-event
  filter helper (CONSUMED in, v0.18.0); cross-bucket
  revocation replication (CONSUMED in, v0.19.0 — see
  ...
  (the four Z-1-K-consensus points remain byte-identical;
...
no on-disk loader is removed, no verifier module is forced to
switch to the registry. Verifiers that already load schemas via
`importlib.resources` continue to function unchanged. The registry
is a **production-target substrate** that Phase-2 deployment will
switch to; This revision lands the substrate, hardens it with the
CAS-pin path. The cutover itself is still Phase-2.

**v0.2.0 is additive relative to v0.1.0:**

- All surfaces (`get` / `put` / `delete` / `list_keys` /
  `snapshot` / `get_by_triple`) remain unchanged. Their
  contracts are preserved byte-equal.
- The additions (`get_with_revision` / `put_with_revision` /
...
- M-2 conformance (additive-only schema evolution): This revision adds no
...
- M-4 conformance (multi-version-aware registry): This revision is orthogonal
...
- All previously shipped surfaces remain unchanged. This revision introduces
...
- The additions (`watch` / `WatchOp` / `WatchEvent` /
...
**v0.5.0 is additive relative to v0.4.0:**

- All previously shipped surfaces remain unchanged.
  This revision introduces no new method on `NatsKvSchemaRegistry`, no new
...
- The addition is a *separate module*
  ...
  LWW + CAS-pin + watch-stream surfaces; it
...
- M-2 conformance (additive-only schema evolution): This revision adds no
  ...
  `_envelope_to_entry` from the backend).
- M-4 conformance (multi-version-aware registry): This revision is orthogonal
...
**v0.6.0 is additive relative to v0.5.0:**

- All previously shipped surfaces remain
  unchanged. This revision introduces no new method on
...
- The envelope codec (`_entry_to_envelope` /
  `_envelope_to_entry`) is left UNCHANGED in. The
  ...
  the codec remain byte-equal to v0.5.0 output (backward
  compatibility invariant); signed envelopes emitted by
  `envelope_with_signature` differ from the output by exactly
  the one optional `signature` slot.
- M-2 conformance (additive-only schema evolution): Tag-1
...
- M-4 conformance (multi-version-aware registry): Tag-1
...
**v0.7.0 is additive relative to v0.6.0:**

- All previously shipped surfaces
  remain unchanged. This revision introduces no new method on
...
- The `wirelang.schemas.entry_signing` module is UNCHANGED. The
  ...
  design that the signing module treats the public key as
  caller-supplied.
- M-2 conformance (additive-only schema evolution): Tag-3
...
- M-4 conformance (multi-version-aware registry): Tag-3
...
**v0.8.0 is additive relative to v0.7.0:**

- All previously shipped surfaces remain unchanged. This revision
  introduces no new method on `NatsKvSchemaRegistry`, no modification to
  `wirelang.schemas.entry_signing`, and no modification to
  `wirelang.identity.kid_resolver` (the surface).
- The addition is a *separate module*
...
- M-2 conformance (additive-only schema evolution): Tag-4
...
- M-4 conformance (multi-version-aware registry): Tag-4
...
- Cross-Review-Zone-1 (Identity-Substrate) non-touched: the four
  Z-1-K-consensus points remain byte-identical. The Tag-4
  `anchor_required` toggle is an *orthogonal* hard-vs-soft policy
  governing the transport-trust step; it is semantically distinct
  from the `VerifyMode.STRICT` signature-policy toggle
  (Z-1-K-Sprint-4-4).
- The synchronous verifier surface (`InMemorySchemaRegistry.lookup`
...
**v0.9.0 is additive relative to v0.8.0:**

- All previously shipped surfaces remain unchanged. This revision
  introduces no new method on
...
- The cache's body-digest helper (`_jcs_body_digest_hex`) is
  byte-equal in shape and output to the Tag-4
...
- M-2 conformance (additive-only schema evolution): Tag-5
  adds NO new envelope field. The schema-registry on-the-wire
  surface (envelope shape, value-schema URI
  `wakir.wirelang.schema-registry-entry/1`, signature-block shape)
  is bit-equal to v0.8.0. M-2 is preserved trivially.
- M-4 conformance (multi-version-aware registry): Tag-5
...
**v0.10.0 is additive relative to v0.9.0:**

- All Tag-1 + Tag-3 + Tag-4 + surfaces remain
  unchanged. This revision introduces no breaking change to
  ...
  byte-equal.
- The additions (`CapabilityPolicy` /
  ...
  entry-signing layer directly; they are not forced onto
  the gate.
- M-2 conformance (additive-only schema evolution): This revision adds no
  ...
  substantiation); the in-process policy bundle uses a
  separate Python dataclass that does NOT yet round-trip through
  that JSON envelope. The two surfaces are intentionally
  separate at: the in-process bundle ships now; the
  on-the-wire serialisation is Phase-3.
- M-4 conformance (multi-version-aware registry): This revision is
...
- Cross-Review-Zone-1 (Identity-Substrate) **non-touched**: the
  four Z-1-K-consensus points remain byte-identical
  after. The gate is curve-agnostic (Z-1-K-Sprint-4-3
...
**v0.11.0 is additive relative to v0.10.0:**

- All Tag-1 / Tag-3 / Tag-4 / Tag-5 / surfaces and
  all Tag-1 / Tag-3 / Tag-4 / Tag-5 / surfaces remain
  unchanged. Their contracts are preserved byte-equal.
- The additions are *operator-CLI* surface extensions
  ...
  in the byte-equal pre-matrix; no caller is forced onto
  the capability path.
- M-2 conformance (additive-only schema evolution): Tag-1
  ...
  is REFERENCED only; the file format is a separate,
...
- M-4 conformance (multi-version-aware registry): This revision is
  ...
  `(registered_by, kid, layer, name)` 4-tuple per contract and
...
- Receipt-shape additivity: the three new optional receipt fields
  ...
  pre-field set continue to read byte-equal pre-existing
  ...
  byte-for-byte across and receipts
  WILL see a difference (the three new keys), but consumers that
  filter to the pre-key set will not. The minor-version
  bump reflects this.
- Cross-Review-Zone-1 (Identity-Substrate) **non-touched**: the four
  Z-1-K-consensus points remain byte-identical after
  . The integration is curve-agnostic at the
  CLI-flag layer (operator supplies the seed; the underlying
  `sign_entry` call is byte-identical to); JCS-free at the
...
- `entry_signing.sign_entry`, `entry_signing.verify_entry_signature`,
  ...
  the pre-unsigned path. The signature block is consumed
  CLI-side only.
...
- Cross-Review-Zone-B **TRIGGERED**: the orchestrator-side
  ...
  the live cluster.
- The in-process `CapabilityPolicyRegistry` remains
  the canonical evaluation surface. This revision ships the
  source-of-truth substrate that materialises it via
  `NatsKvCapabilityPolicyBackend.snapshot_registry`. The Sprint-4
  `check_registered_by_capability` function is byte-unchanged
  ...
  JSON (path) or materialised from the persistent
  bucket (path).
- The publisher-CLI `--capability-registry` flag is
  byte-unchanged: it still reads operator-local JSON. A future
  `--capability-bucket` flag that reads policies from the
  bucket is a candidate; Sprint-5
  This revision is the substrate, not the CLI integration.
- The backend ships PUT (LWW) only. A CAS-pin tier
  (`put_with_revision`) is reserved as a Phase-3 slot mirroring
  the schema-registry CAS-pin pattern.
- The backend ships full-snapshot only. A
  watch-stream tail (`watch()` / `WatchEvent` /
  `LiveCapabilityPolicySnapshot`) is reserved as a Phase-3 slot
  mirroring the schema-registry watch-stream
  pattern.
...
**v0.13.0 is additive relative to v0.12.0:**
...
- The receipt's `gate_policy_source` is `"file"` for the Sprint-5
  file path, `"bucket"` for the new bucket
...
- The schema-registry bucket (`wakir-schemas`) write path is
  ...
  the file-source short-circuit (verified by
  T-SR-PUB-CB-03..05).
...
**v0.14.0 is additive relative to v0.13.0:**
...
- All LWW surfaces (`put` / `get` / `get_by_pair` /
...
- Cross-Review-Zone-1 (Identity-Substrate) non-touched: the
  ...
  verifier contract. The four Z-1-K-consensus points
...
- Cross-Review-Zone-B (Kai bucket inventory) non-touched: no new
  ...
  byte-unchanged; the paired-update memo remains
  the canonical Z-B trigger.
- The publisher-CLI `--capability-bucket` flag is
  read-only (`snapshot_registry()`) so the CAS-pin write path is
  not reachable from the operator CLI in. A
  `--capability-bucket-cas-rotate` (or similar) subcommand surface
  is a candidate, explicitly NOT shipped in.
- The schema-registry CAS-pin surface is
  byte-unchanged. The surface is a parallel module-
...
- The Phase-3 capability-policy watch-stream slot (full-snapshot
  only in Tag-2/3/4; live-tail is Phase-3) and the
  Phase-3 multi-replica CAS-quorum slot (single-replica
  `replicas=1` in Tag-2/3/4 configuration) remain
  reserved.
...
- The additions to
  ...
  - New enum `CapabilityPolicyWatchOp` with values `PUT`, `DELETE`,
    ...
    `WatchOp`; the class identity is module-local
  ...
  - New live-tail consumer dataclass
    ...
    frozen `CapabilityPolicyRegistry` view in sorted-
...
- Cross-Review-Zone-1 (Identity-Substrate) non-touched: the
  ...
  Z-1-K-consensus points (kid-Resolver-Shape, JCS-
...
- Cross-Review-Zone-B (Kai bucket inventory) non-touched: no new
  bucket; the `wakir-capability-policies` bucket configuration is
  byte-unchanged; the paired-update memo remains the
  canonical Z-B trigger.
- The publisher-CLI `--capability-bucket` flag is
  read-only (`snapshot_registry`) and the Tag-5
  watch-stream is NOT reachable from the operator CLI in.
...
- Spec semver bump 0.14.0 → 0.15.0 reflects the additive minor
  ...
  new §5.14 "Watch-stream operational contract (
  additive)" subsection, and the new §6.13
...
content). The additions (§5.8, §6.5, change-log v0.6.0 entry,
Phase-2 boundary updates) use role-strings only (`Reza`, `Tomás`,
`Mira`, `Aisha` appear in outbox documents and optional
cross-review memos, not in this spec; the spec mentions only
`wakir.*` URIs and module-path references).

The additions (§5.9, §6.6, change-log v0.7.0 entry, §1.2 block, §7
cross-references update, §8 compatibility statement update) have
been swept identically — only role-strings, module-path references,
`wakir.*` URIs, and IETF / RFC references appear in the spec body.
No external-tool clear-name leakage and no
internal-persona-clear-name leakage in the spec body.

The additions (§5.10, §6.7, change-log v0.8.0 entry, §1.2 block, §5.3
lands-update, §7 cross-references update — transport-fetch slot
CONSUMED, §8 compatibility statement update for v0.7.0 → v0.8.0) have
been swept identically — only role-strings (none in this spec body),
...
(RFC 8615 well-known namespace, RFC 8785 JCS, RFC 8032 Ed25519, V-908
spec sections) appear in the spec body. No external-tool clear-name
leakage and no internal-persona-clear-name leakage in the spec body
additions.

The additions (§5.11, §6.8, change-log v0.9.0 entry, §1.2 block,
§5.3 lands-update — added to the §5.3 header, AIP-document
signature-verification cache tier item moved from Phase-3-reserved
to CONSUMED-in-Sprint-4-, §7 cross-references update —
verification-cache tier slot CONSUMED, §8 compatibility statement
update for v0.8.0 → v0.9.0) have been swept identically — only
role-strings (none in this spec body), module-path references
(`wirelang.identity.aip_signature_verification_cache`,
`wirelang.identity.verify_aip_signature`,
`wirelang.identity.aip_signing._jcs_canonicalize`), `wakir.*` URIs
(carried unchanged from prior tags), and IETF / RFC references (RFC
8785 JCS, RFC 8032 Ed25519, V-908 spec sections) appear in the spec
body. No external-tool clear-name leakage and no
internal-persona-clear-name leakage in the spec body additions.

The additions (§5.12, §6.9, change-log v0.10.0 entry, §1.2 block, §5.3 lands-update — added
to the §5.3 header, `registered_by`-capability-gating item moved from
follow-up-Phase-2-slot (§5.8 boundary) to CONSUMED-in-Sprint-4-, §7 cross-references update
— `registered_by`-capability-gating slot CONSUMED, §8 compatibility statement update for
v0.9.0 → v0.10.0) have been swept identically — only role-strings (none in this spec body),
module-path references (`wirelang.schemas.registered_by_capability`,
`wirelang.schemas.entry_signing`, `wirelang.schemas.registry_nats_kv_backend`), `wakir.*`
URIs (`wakir.wirelang.schema-registry-entry/1`, `wakir-capability-policies` reserved bucket
name, carried unchanged from prior tags), and Python-stdlib/IETF/RFC references
(:mod:`fnmatch` grammar, RFC 8032 Ed25519, `wirelang/schemas/layer-3-capability-token.json`
Biscuit-v3 JSON envelope schema) appear in the spec body. No external-tool clear-name
leakage and no internal-persona-clear-name leakage in the spec body additions.

The additions (§5.13, §6.10, change-log v0.11.0 entry, §5.3
publisher-CLI-capability-integration-slot CONSUMED, §5.6 publisher-CLI
capability-flag cross-reference, §5.12 boundary publisher-CLI item CONSUMED, §7
publisher-CLI-integration-of-Sprint-4-Tag-6-gate slot CONSUMED, §8 compatibility
statement update for v0.10.0 → v0.11.0) have been swept identically — only
role-strings (none in this spec body), module-path references
(`wirelang.schemas.publisher_cli`, `wirelang.schemas.entry_signing`,
`wirelang.schemas.registered_by_capability`), `wakir.*` URIs (`wakir-schemas`
bucket name, `wakir.wirelang.schema-registry-entry/1` envelope schema,
`wakir-capability-policies` reserved Phase-3 bucket name,
`wakir-schema-registry` CLI program name, carried unchanged from prior tags),
and IETF/RFC references (RFC 8032 Ed25519, RFC 8259 JSON, RFC 3339 timestamps,
RFC 8785 JCS). The capability-registry JSON file path examples use generic
placeholder paths (`~/.config/wakir/biscuit-root-1.seed`,
`capability-registry.json`) and the canonical operator examples (`wirelang-eng`,
`biscuit-root-1`) are role-strings consistent with prior tag conventions. No
external-tool clear-name leakage and no internal-persona-clear-name leakage in
the spec body additions.

The additions (§5.14, §6.11, change-log v0.12.0 entry, §5.12 boundary
persistent-capability-policy-distribution item CONSUMED, §5.13
boundary persistent-capability-policy-distribution item CONSUMED, §7
Phase-3-Reservation persistent capability-policy-distribution slot
CONSUMED, §8 compatibility statement update for v0.11.0 → v0.12.0)
...
IETF/RFC references (RFC 3339 timestamps, RFC 8259 JSON, RFC 8785 JCS
— though the persistent policy envelope is NOT a JCS-anchored digest
at the policy-envelope axis). The persistent-bucket key prefix
(`capability-policies/`), the policy-pair format
...
strings (`default`, `tier-1-ingress`, `wire-scope`, `identity-scope`,
`alpha`, `beta`, `gamma`) are role-strings / operator-side identifiers
consistent with prior tag conventions. No external-tool clear-name
leakage and no internal-persona-clear-name leakage in the spec body
additions.

The additions (§5.13 "Bucket policy source" subsection, §6.10
"Bucket policy source tests" addendum, change-log v0.13.0 entry,
§5.13 boundary item "future `--capability-bucket`" CONSUMED, §5.14
boundary item "publisher-CLI integration" CONSUMED, §7
Phase-3-Reservation "publisher-CLI `--capability-bucket`
integration" CONSUMED, §8 compatibility statement update for
v0.12.0 → v0.13.0) have been swept identically — only module-path
references (`wirelang.schemas.publisher_cli`,
`wirelang.schemas.capability_policy_nats_kv_backend`,
`wirelang.schemas.registered_by_capability`), `wakir.*` URIs
(`wakir-capability-policies` carried unchanged from; no new bucket
added), the new flag names (`--capability-bucket`,
...
The additions (§5.14 "CAS-pin operational contract" subsection,
§6.12 capability-policy CAS-pin test inventory, change-log v0.14.0
entry, §5.14 boundary item "future CAS-pinned upsert path"
CONSUMED, §7 Phase-2 capability-policy CAS-pin slot CONSUMED, §8
compatibility statement update for v0.13.0 → v0.14.0) have been
swept identically — only module-path references
(`wirelang.schemas.capability_policy_nats_kv_backend`,
`wirelang.schemas.registry_nats_kv_backend`,
`wirelang.schemas.registered_by_capability`), `wakir.*` URIs
(`wakir-capability-policies` carried unchanged from; no new bucket
added), the new exception class name
...
The additions (§5.14 "Watch-stream operational contract (additive)"
subsection, §5.14 "Boundary: This revision does NOT ship (explicit)"
subsection, §6.13 capability-policy watch-stream test inventory,
change-log v0.15.0 entry, §5.14 boundary item "live tail reserved as
a Phase-3 slot" CONSUMED, §7 Phase-3 capability-policy watch-stream
slot CONSUMED, §7 "No watch-stream on the capability-policy bucket"
boundary CONSUMED, §8 compatibility statement update for v0.14.0 →
v0.15.0) have been swept identically — only module-path references
(`wirelang.schemas.capability_policy_nats_kv_backend`,
`wirelang.schemas.registry_nats_kv_backend`,
`wirelang.schemas.registered_by_capability`), `wakir.*` URIs
(`wakir-capability-policies` carried unchanged from; no new bucket
added), the new public class names (`CapabilityPolicyWatchOp`,
`CapabilityPolicyWatchEvent`, `LiveCapabilityPolicySnapshot`), the
new public function name (`open_capability_policy_watch_stream`), the
new internal helper / handle names
(`_CapabilityPolicyWatchStreamHandle`,
...
`biscuit-root-2`, `biscuit-root-r1`, `biscuit-root-r2`, `rotation-1`,
`rotation-2`, `default`), and the nats-py adapter shape names
(`watchall`, `watch`, `updates`, `stop`, `__aiter__`, `__anext__`)
are role-strings / operator-side identifiers / public API names
consistent with prior tag conventions. No external-tool clear-name
leakage and no internal-persona-clear-name leakage in the spec body
additions.

The additions (§10 "Persona-Engine-Format integration" + changelog
v0.32.0 entry + version-line bump 0.31.0 → 0.32.0 + new-section
opener anchor) have been swept identically — only role-strings
(`pengine-eng`, `container-ops-slot`, `wat-eng-slot`,
`identity-eng-slot`, `hr-slot`), module-path references
(`wirelang/specs/persona-engine-format-spec.md`,
...
`persona_hash::compute_persona_hash_from_canonical`), `wakir.*` URIs
(`wakir.dev/schemas/persona-claude-native.json`,
`wakir.dev/schemas/wakir-persona-v1.json`,
`wakir-persona-state-{persona_id}` bucket-name family RESERVED),
persona slugs (`mira`, `cto`, `hr`, `cfo`, `comms`, `internal-audit`,
`dev-engineering`, `reza`, `kai`, `pengine`, `frontend`, `qa`, `sre`
— these are the role-string slugs per Brand-Guide §9, NOT clear
personal names), the new public function names
(`map_claude_native_to_wakir_v1`, `wakir_persona_hash`,
`jcs_canonicalise_wakir_persona_v1`), the new error-class name
(`PersonaEngineFormatError`), the schema-versions
(`wakir-persona-v1`, `persona-claude-native`, `persona-v1`), the
canonical_subset embedded surface (`name`, `description`, `tools`,
`schema_version`, `identity_pinned`, `cross_review_zones`,
`authority`, `hierarchy`), and the lifecycle state-names
(`uninstantiated`, `spawning`, `running`, `despawning`, `recovered`,
`migrated`) are role-strings / operator-side identifiers / public API
names consistent with prior tag conventions. No external-tool
clear-name leakage and no internal-persona-clear-name leakage in the
spec body additions.

## 10. Persona-Engine-Format integration

### 10.1 Scope

This section anchors the Persona-Engine-Format integration into the
schema-registry spec at v0.32.0. The companion spec
`wirelang/specs/persona-engine-format-spec.md` v1.0 holds the full
normative content; this section is a registry-side index of the new
schema-registry surfaces.
...
sprint; keeps them as filesystem-only artefacts to keep the
...
This is the **load-bearing decision** of: the framework-native
format does not fork V-907; it embeds it.
...
| Zone | Partner | touch | ack required |
|---|---|---|---|
| J (container-bridge) | container-ops-slot | shape-only (image template, labels, env vars in §3.5 of persona-engine-format-spec) | yes, before Tag-4 |
...
| L (identity-substrate) | identity-eng-slot | reservation-only (synthesised hierarchy default in §4.3) | not until Tag-3+ |
| B (NATS-KV × Wirelang) | container-ops-slot (bucket inventory) | reservation-only (`wakir-persona-state-{persona_id}` family documented; no bucket configured) | not until Tag-4 |
| HR (governance) | hr-slot | active (synthesis defaults HR-slot-ratification-pending) | yes, before Tag-2 |

### 10.6 Compatibility statement (Tag-1 → v0.32.0)

v0.32.0 is purely additive over v0.31.0:

- All..schema-registry surfaces (`get` / `put` /
...
- The capability-policy backend remains byte-unchanged.
- The SPIFFE-cross-trust-domain-bridge, Multi-Org-
  Attestation backend, replicators, and capability-attenuation-
  chain-verifier remain byte-unchanged.
- The marker-composition reducer and persistent
  marker-stack NATS-KV backend remain byte-unchanged.
...
## 11. Operator-CLI annex (Teil A)

### 11.1 Scope

This annex anchors the Teil A operator-facing marker-stack
reducer CLI into the schema-registry spec at v0.35.0. The CLI
wraps the in-memory reducer
(`wirelang.federation.marker_composition.reduce_marker_stack`)
plus the durable backend
...
Live NATS connections are operator-hand. All Teil A CLI tests
are hermetic and inject a `_BackendStub` (no NATS, no nats-py
import, no filesystem mutation). The live factory uses a
deferred `nats` import so the module loads under test without
nats-py on the path.

### 11.8 Compatibility statement (Tag-3 → v0.35.0)

v0.35.0 is purely additive over v0.34.0:

- All Tag-3 / surfaces (`reduce_marker_stack`,
...
- All Tag-1 / surfaces (`detect_replay`,
...
- The `MultiOrgAttestationReplicator`
