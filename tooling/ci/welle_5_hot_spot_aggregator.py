...
"""Welle-5 Hot-Spot Aggregator -- Tag-48 (Tomas / Henrik-Pre-Mortem-Folge).

Background
----------

Henrik's Tag-44 + Tag-45 Pre-Mortem-Mitigation-Map identifies Welle-5
(``lifecycle_state_machine`` / ``persona-engine-fsm`` in its role as
the canonical persona-lifecycle FSM) as a **propagation-target Hot-Spot**
for the KW-25..KW-27 cutover marathon. Welle-5 is downstream from
Welle-3 (audit-trail) and Welle-4 (state-backing), and the propagation
matrix says: a red Welle-3 or red Welle-4 pre-conditionally blocks
Welle-5 readiness.

Welle-5 hot-spot characterisation
---------------------------------

The structural risk for Welle-5 is **FSM-Phantom-Detection live**:
the lifecycle FSM has a known historical pattern (Henrik Tag-22
A2 finding) where a phantom-transition can appear during cutover-
window state replay -- a transition that exists in the canonical
trace but does not match any FSM edge in the live engine. The
Tag-22 hermetic A2 coverage fixed the static case; the cutover-
window introduces a *live-replay* variant that needs daily detection.

Welle-5 propagates downstream into Welle-7 (``recovery_workflow``)
because the recovery-workflow replays the FSM canonical trace; a
phantom-transition in the FSM cascades into recovery-divergence.

The four checks
---------------

* **Check 1 -- Self-Reference-Trap-Pre-Detection**: verify the
  Welle-5 cutover-smoke substrate
  (``scripts/phase-3c/welle-5-lifecycle-state-machine-cutover-smoke.{py,sh}``
  and the ``persona-engine-fsm`` crate) does NOT consume its own
  emitted state-trace as input during the cutover-window. Patterns:
  ``replay_own_trace``, ``read_fsm_trace_for_self``,
  ``self_referential_fsm_loop``, ``fsm_self_oracle``.

* **Check 2 -- FSM-Phantom-Detection live indicator**: read
  ``state/welle-5-fsm-phantom-detection.json`` (produced by the weekly
  Welle-5-validation workflow's live-replay step). The schema field
  ``phantom_transition_count`` and ``phantom_detection_status`` map
  to the tri-state. When red, propagation block fires for downstream
  Welle-{7}.

* **Check 3 -- Independent-Oracle-Probe** (Henrik Tag-38 Caution):
  cross-validate the FSM output against
...
* **Check 4 -- IIA-1130-Pre-Auditor-Decision-Tracking**: verify
  ``state/welle-5-pre-auditor-decision.json`` exists, parses, has
  required fields, and decision is APPROVED. Welle-5 is on Henrik's
  IIA-1130 list per Tag-45 Mitigation-Map.
...
# Downstream wellen pre-conditionally blocked by a red welle-5
# FSM-phantom-detection. Documented in Henrik Tag-44+45 Pre-Mortem
...
def _propagation_block(
    check2_status: str, phantom_detection: str
) -> dict | None:
    """Build the cross-welle-propagation block when applicable.

    Returns ``None`` when no propagation flag fires.
    """
    if check2_status == "red" and phantom_detection == "red":
        return {
            "trigger": "welle-5-fsm-phantom-detection-red",
            "pre_conditional_blocked": list(DOWNSTREAM_PROPAGATION_WELLEN),
            "rationale": (
                "Welle-5 lifecycle_state_machine FSM-phantom-detection "
...
def build_envelope(env: Mapping[str, str]) -> dict:
    ...
    Inputs (env-vars):
      ...
      * CHECK2_STATUS -- FSM-phantom-detection live indicator.
      * CHECK3_STATUS -- independent-oracle-probe.
      * CHECK4_STATUS -- iia-1130-pre-auditor-decision-tracking.
      * WELLE_5_FSM_PHANTOM_DETECTION -- last-known phantom-detection
    ...
    steps = {
        ...
        "check_2_fsm_phantom_detection_live": _normalise(
    ...
    propagation = _propagation_block(
        steps["check_2_fsm_phantom_detection_live"],
        _normalise(env.get("WELLE_5_FSM_PHANTOM_DETECTION")),
    )
    ...
    envelope = {
        "schema_version": 1,
        "workflow": "phase-3c-welle-5-hot-spot-probe",
        "welle": 5,
        ...
        "cross_substrate_links": {
            ...
            "welle_5_pre_auditor_decision_file": (
...
def main(argv: list[str]) -> int:
    p = argparse.ArgumentParser(
        description=(
            "Aggregate the four Welle-5 hot-spot indicator results "
