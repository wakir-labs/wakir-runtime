...
"""Wire the Welle-4 audit-trail-anchor for the persona-engine producer.

Tag-72 Welle-4 State-File Audit-Trail-Anchor-Integration (Tomás,
Cross-Review-Zone-K). Mirror of the Tag-69 Welle-1 / Tag-70 Welle-2 /
Tag-71 Welle-3 helpers, parameterised to Welle-4 (State-Backing-
Snapshot-Restore).

Cross-coord with Selin (Tag-72 Welle-4 producer):

  * Tomás (this module): hash-construction over the canonical
    Welle-4 bundle, envelope emit, snapshot-restore-Pflicht-Flag
    tracking surface. Stdlib-only. No OTS calendar call.
  * Selin (engine.py, welle=4 path): triggers on State-Backing-
    Snapshot-Restore-Cutover-Smoke-Green, calls this helper for the
    hash-side, writes the result into
    ``state/welle-4.json:audit_trail_anchor``.

The hash construction matches
``emit_manifest_hash_ots_marker.compute_welle_4_audit_anchor_hash``
exactly — the two entry points use the same underlying function via
import. Kept separate from the Tag-69 / Tag-70 / Tag-71
...
Snapshot-Restore-Pflicht-Flag-Tracking
--------------------------------------

Welle-4 carries the State-Backing-Snapshot-Restore-Pflicht-Flag
(Amara Tag-67 state-file conventions). The envelope exposes a
``snapshot_restore_pflicht_tracking`` block so the downstream
observability surface can dispatch the restore-Pflicht gate
without re-reading the bundle. **No enforcement here** — the
helper only surfaces tracking; the gate itself lives in Henrik's
Internal-Audit + Amara-QA workflows.
...
::
    ...
    python3 tooling/ci/wire_welle_4_audit_trail_anchor.py \\
        --welle-4-rollup    state/welle-4.json \\
        --welle-4-sign-off  state/welle-4-sign-off.json \\
        --welle-4-validation state/welle-4-validation-last-verdict.json \\
        --welle-4-pre-auditor state/welle-4-pre-auditor-decision.json \\
        --envelope-out tooling/ots/markers/welle-4-audit-anchor.json
...
::
  ...
  {
    "schema_version": 1,
    "kind": "welle-4-audit-trail-anchor-producer-envelope",
    "welle_number": 4,
    ...
    "snapshot_restore_pflicht_tracking": {
      ...
      "snapshot_restore_status": "pending|executed|exempt|unknown",
      "snapshot_restore_iso": "<iso-or-empty>",
      "snapshot_restore_evidence_ref": "<free-form-or-empty>"
    },
    "emitted_at_utc": "<iso>",
    "actor": "<actor>",
    "anchors": { ... },
...
def _import_emit_helper():
    """Import the Tag-57/Tag-69/Tag-70/Tag-71/Tag-72 OTS-emit module."""
...
ENVELOPE_KIND: str = "welle-4-audit-trail-anchor-producer-envelope"
WELLE_NUMBER: int = 4
...
def build_envelope(
    *,
    ...
    snapshot_restore_tracking: dict,
) -> dict:
    """Assemble the producer-facing envelope dict.

    ``snapshot_restore_tracking`` is the derived Welle-4 tracking
    block (see emit_helper.derive_snapshot_restore_tracking). The
    block is surfaced on the envelope so Selin's producer + the
    downstream observability surface can dispatch the restore-
    Pflicht gate without re-reading the bundle.
    """
    ...
    return {
        ...
        "snapshot_restore_pflicht_tracking": dict(snapshot_restore_tracking),
...
def emit_envelope(
    *,
    ...
    """Load the Welle-4 bundle, compute the anchor, return the envelope.

    This is the importable Python entry-point for tests and for
    Selin's Tag-72 producer (which can in principle import this
    ...
    bundle = emit_module.load_welle_4_bundle(
    ...
    anchor_hash = emit_module.compute_welle_4_audit_anchor_hash(bundle)
    tracking = emit_module.derive_snapshot_restore_tracking(bundle)
    ...
    return build_envelope(
        ...
        actor=actor,
        now_utc=now_utc,
        snapshot_restore_tracking=tracking,
    )


def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="wire_welle_4_audit_trail_anchor",
        description=(
            "Compute the Welle-4 audit-trail-anchor SHA-256 from the "
            "sign-off-record bundle and emit a producer-facing envelope "
            "for Selin's Tag-72 persona-engine batch-writer (State-"
            "Backing-Snapshot-Restore). Surfaces the snapshot-restore-"
            "Pflicht-Flag-tracking block (Amara Tag-67)."
        ),
    )
    parser.add_argument(
        "--welle-4-rollup",
        type=Path,
        required=True,
        help="Path to state/welle-4.json (rollup state-file).",
    )
    parser.add_argument(
        "--welle-4-sign-off",
        type=Path,
        required=True,
        help="Path to state/welle-4-sign-off.json.",
    )
    parser.add_argument(
        "--welle-4-validation",
        type=Path,
    ...
    if not args.welle_4_rollup.is_file():
        print(
            f"wire_welle_4_audit_trail_anchor: --welle-4-rollup not "
    ...
    if not args.welle_4_sign_off.is_file():
        print(
            f"wire_welle_4_audit_trail_anchor: --welle-4-sign-off not "
            f"a file: {args.welle_4_sign_off}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        envelope = emit_envelope(
            ...
            validation_path=args.welle_4_validation,
            pre_auditor_path=args.welle_4_pre_auditor,
            actor=args.actor,
            now_utc=now_utc,
        )
    except (OSError, json.JSONDecodeError) as exc:
        print(
            f"wire_welle_4_audit_trail_anchor: bundle read/parse "
            f"error: {exc}",
            file=sys.stderr,
        )
        return 1
        ...
        print(
            f"wire_welle_4_audit_trail_anchor: bundle shape error: "
            f"{exc}",
            file=sys.stderr,
        )
        return 1
    ...
    tracking = envelope["snapshot_restore_pflicht_tracking"]
    print(
        f"wire_welle_4_audit_trail_anchor: welle=4 "
