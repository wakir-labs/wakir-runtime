...
"""Wire the Welle-2 audit-trail-anchor for the persona-engine producer.

Tag-70 Welle-2 State-File Audit-Trail-Anchor-Integration (Tomás,
Cross-Review-Zone-K). Mirror of the Tag-69 Welle-1 helper, parameter-
ised to Welle-2 (Doppelbetrieb-Sealing) bundle.

Cross-coord with Selin (Tag-70 Welle-2 producer):

  * Tomás (this module): hash-construction over the canonical
    Welle-2 bundle, envelope emit. Stdlib-only. No OTS calendar call.
  * Selin (engine.py, welle=2 path): triggers on Doppelbetrieb-Seal-
    Marker-Fired, calls this helper for the hash-side, writes the
    result into ``state/welle-2.json:audit_trail_anchor``.

The hash construction matches
``emit_manifest_hash_ots_marker.compute_welle_2_audit_anchor_hash``
exactly — the two entry points use the same underlying function via
import. Kept separate from the Tag-69 ``wire_welle_1_*`` helper to
keep the producer-facing surface narrow (one envelope kind per welle)
and to permit a future welle-specific divergence without forcing a
...
::
    ...
    python3 tooling/ci/wire_welle_2_audit_trail_anchor.py \\
        --welle-2-rollup    state/welle-2.json \\
        --welle-2-sign-off  state/welle-2-sign-off.json \\
        --welle-2-validation state/welle-2-validation-last-verdict.json \\
        --welle-2-pre-auditor state/welle-2-pre-auditor-decision.json \\
        --envelope-out tooling/ots/markers/welle-2-audit-anchor.json
...
::
  ...
  {
    "schema_version": 1,
    "kind": "welle-2-audit-trail-anchor-producer-envelope",
    "welle_number": 2,
    ...
    "bundle_paths": {
      ...
      "validation": "state/welle-2-validation-last-verdict.json",
      "pre_auditor": "state/welle-2-pre-auditor-decision.json"
    },
    "emitted_at_utc": "<iso>",
    "actor": "<actor>",
    "anchors": {
...
def _import_emit_helper():
    """Import the Tag-57/Tag-69/Tag-70 OTS-emit module without packaging."""
...
ENVELOPE_KIND: str = "welle-2-audit-trail-anchor-producer-envelope"
WELLE_NUMBER: int = 2
...
def emit_envelope(
    *,
    ...
    """Load the Welle-2 bundle, compute the anchor, return the envelope.
    ...
    bundle = emit_module.load_welle_2_bundle(
    ...
    anchor_hash = emit_module.compute_welle_2_audit_anchor_hash(bundle)
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="wire_welle_2_audit_trail_anchor",
        description=(
            "Compute the Welle-2 audit-trail-anchor SHA-256 from the "
            "sign-off-record bundle and emit a producer-facing "
            "envelope for Selin's Tag-70 persona-engine batch-writer "
            "(Doppelbetrieb-Sealing)."
        ),
    )
    parser.add_argument(
        "--welle-2-rollup",
        type=Path,
        required=True,
        help="Path to state/welle-2.json (rollup state-file).",
    )
    parser.add_argument(
        "--welle-2-sign-off",
        type=Path,
        required=True,
        help="Path to state/welle-2-sign-off.json.",
    )
    parser.add_argument(
        "--welle-2-validation",
        type=Path,
    ...
    if not args.welle_2_rollup.is_file():
        print(
            f"wire_welle_2_audit_trail_anchor: --welle-2-rollup not "
    ...
    if not args.welle_2_sign_off.is_file():
        print(
            f"wire_welle_2_audit_trail_anchor: --welle-2-sign-off not "
            f"a file: {args.welle_2_sign_off}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        envelope = emit_envelope(
            ...
            validation_path=args.welle_2_validation,
            pre_auditor_path=args.welle_2_pre_auditor,
            actor=args.actor,
            now_utc=now_utc,
        )
    except (OSError, json.JSONDecodeError) as exc:
        print(
            f"wire_welle_2_audit_trail_anchor: bundle read/parse "
            f"error: {exc}",
            file=sys.stderr,
        )
        return 1
        ...
        print(
            f"wire_welle_2_audit_trail_anchor: bundle shape error: "
            f"{exc}",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        f"wire_welle_2_audit_trail_anchor: welle=2 "
