...
"""
Tag-73 hermetic verifier for the Operator-Hand Welle-5 Day-of-
Recipe-Patch doc.

Source doc:
  docs/operations/operator-hand-welle-5-eve-recipe-patch.md

Welle-5 is the second Phase-3c marathon welle with Pre-Auditor-
Section-11-Discipline (IIA-1130), after Welle-3 (Tag-71-Patch
precedent). Unlike Welle-3 (single audit-stream-self-reference hot
spot), Welle-5 carries two intertwined hot-spot axes:

* fsm-phantom            (FSM-Phantom-Transition class, Tag-22 A2
                          lineage, Henrik Tag-44/45 re-probe)
* capability-token-rotation
                         (Capability-Token rotation drift between
                          Rust-FSM-resolver and Python-FSM-resolver)
...
consistent, plus the four Welle-5-specific Rollback-Pfade (one more
than Welle-3 to capture the KW-26-Doppel-Welle-Decouple branch).
...
H4  Steps P1..P4 are Pre-Sequence (in §1); step P5 is Post-Sequence
    (in §3).
H5  Four Welle-5-specific Rollback-Pfade R-W5-A, R-W5-B, R-W5-C,
    R-W5-D are present (one more than Welle-3 to capture the
    KW-26-Doppel-Welle-Decouple branch).
H6  Three Welle-5-specific AR-Hand-Touchpoints TP-W5-1, TP-W5-2,
    TP-W5-3 are present.
H7  Aggregate Welle-5-Day Verdict-Roll-up table is present in §4
    and lists at minimum seven verdict markers including
    WELLE-5-VERDICT-CUTOVER-DONE, WELLE-5-VERDICT-NO-GO, and
    WELLE-5-VERDICT-DOPPEL-DECOUPLE (Doppel-Welle-specific).
H8  Pre-Auditor-State-File path `state/welle-5-pre-auditor-
...
H11 Both hot-spot-axis names are referenced explicitly:
    'fsm-phantom' AND 'capability-token-rotation'.
H12 The two Welle-5-specific state-files are referenced:
    `state/welle-5-fsm-phantom-detection.json` AND
    `state/welle-5-capability-token-rotation-drift.json`.

Soft-checks (warn-only, exit-0)
================================

S1  Welle-5-Day date is convergent (KW-26-Mi 2026-06-17). Doc
    should reference this explicitly.
S2  Welle-3-Recipe-Patch (Tag-71) is referenced as precedent.
S3  KW-26-Doppel-Welle constraint with Welle-4 partner is
    documented.
"""
...
DEFAULT_DOC = "docs/operations/operator-hand-welle-5-eve-recipe-patch.md"
...
REQUIRED_R_PATHS = ["R-W5-A", "R-W5-B", "R-W5-C", "R-W5-D"]
REQUIRED_TP = ["TP-W5-1", "TP-W5-2", "TP-W5-3"]
REQUIRED_VERDICTS = [
    "WELLE-5-VERDICT-CUTOVER-DONE",
    "WELLE-5-VERDICT-NO-GO",
    "WELLE-5-VERDICT-DOPPEL-DECOUPLE",
]
PRE_AUDITOR_STATE_FILE = "state/welle-5-pre-auditor-decision.json"
FSM_PHANTOM_STATE_FILE = "state/welle-5-fsm-phantom-detection.json"
CAP_TOKEN_DRIFT_STATE_FILE = (
    "state/welle-5-capability-token-rotation-drift.json"
)
IIA_ANCHOR = "IIA-1130"
...
def verify(doc_path: Path) -> VerifyResult:
    ...
    # H5 -- four Welle-5-specific rollback paths.
    ...
    # H6 -- three Welle-5-specific touchpoints.
    ...
    if "§4" not in sections:
        ...
        verdict_rows = [
            ...
            if "->" in line and "WELLE-5-VERDICT-" in line
        ]
    ...
    # H12 -- two Welle-5-specific state-files referenced.
    if FSM_PHANTOM_STATE_FILE not in text:
        result.hard_fail(
            f"H12: state file path {FSM_PHANTOM_STATE_FILE} not referenced"
        )
    if CAP_TOKEN_DRIFT_STATE_FILE not in text:
        result.hard_fail(
            f"H12: state file path {CAP_TOKEN_DRIFT_STATE_FILE} not referenced"
        )

    # S1 -- convergent Welle-5-Day date check.
    if "2026-06-17" not in text or "KW-26" not in text:
        result.soft_warn(
            "S1: doc should reference Welle-5-Day convergent anchor "
            "(KW-26-Mi 2026-06-17)"
        )

    # S2 -- Welle-3-Recipe-Patch (Tag-71) precedent reference.
    if "Tag-71" not in text or "welle-3-eve-recipe-patch" not in text.lower():
        result.soft_warn(
            "S2: doc should reference Welle-3-Recipe-Patch (Tag-71) as precedent"
        )

    # S3 -- KW-26-Doppel-Welle constraint with Welle-4.
    if "Doppel-Welle" not in text or "Welle-4" not in text:
        result.soft_warn(
            "S3: doc should document KW-26-Doppel-Welle constraint with "
            "Welle-4 partner"
        )
...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Tag-73 verifier for Welle-5 Day-of-Recipe-Patch doc."
    )
    parser.add_argument(
        "--doc",
        type=Path,
    ...
    print(f"verify_welle_5_recipe_patch_doc :: {doc_path}")
    ...
    if result.ok:
        print("VERDICT :: WELLE-5-RECIPE-PATCH-DOC-CLEAN")
        return 0
    print("VERDICT :: WELLE-5-RECIPE-PATCH-DOC-DEFECT")
    return 1
