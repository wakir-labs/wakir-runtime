...
"""
Tag-74 hermetic verifier for the Operator-Hand Welle-6 Day-of-
Recipe-Patch doc.

Source doc:
  docs/operations/operator-hand-welle-6-eve-recipe-patch.md

Welle-6 is the third Phase-3c marathon welle with Pre-Auditor-
Section-11-Discipline (IIA-1130), after Welle-3 (Tag-71-Patch) and
Welle-5 (Tag-73-Patch). Welle-6 carries two intertwined hot-spot
axes plus the KW-27-Doppel-Welle directional-cascade asymmetry:

* nats-subscribe-mode      (NATS-Subscribe-Mode-Live-Check, Tag-46
                            A8 NATS-JetStream-Loss-Recovery lineage)
* subscribe-loop-cascade   (Subscribe-Loop-Cascade-into-Welle-7
                            replay-divergence drift between
                            Rust-resolver and Python-resolver)
...
consistent, plus the four Welle-6-specific Rollback-Pfade (including
the KW-27-Doppel-Welle-Decouple branch with cascade-directionality
asymmetry vs. Welle-5).
...
H4  Steps P1..P4 are Pre-Sequence (in §1); step P5 is Post-Sequence
    (in §3).
H5  Four Welle-6-specific Rollback-Pfade R-W6-A, R-W6-B, R-W6-C,
    R-W6-D are present (matching Welle-5 pattern; D captures the
    KW-27-Doppel-Welle-Decouple branch with directional-cascade
    asymmetry).
H6  Three Welle-6-specific AR-Hand-Touchpoints TP-W6-1, TP-W6-2,
    TP-W6-3 are present.
H7  Aggregate Welle-6-Day Verdict-Roll-up table is present in §4
    and lists at minimum seven verdict markers including
    WELLE-6-VERDICT-CUTOVER-DONE, WELLE-6-VERDICT-NO-GO, and
    WELLE-6-VERDICT-DOPPEL-DECOUPLE (Doppel-Welle-specific).
H8  Pre-Auditor-State-File path `state/welle-6-pre-auditor-
...
H11 Both hot-spot-axis names are referenced explicitly:
    'nats-subscribe-mode' AND 'subscribe-loop-cascade'.
H12 The two Welle-6-specific state-files are referenced:
    `state/welle-6-subscribe-mode-live.json` AND
    `state/welle-6-cascade-into-welle-7-drift.json`.

Soft-checks (warn-only, exit-0)
================================

S1  Welle-6-Day date is convergent (KW-27-Mi 2026-06-24). Doc
    should reference this explicitly.
S2  Welle-3-Recipe-Patch (Tag-71) AND Welle-5-Recipe-Patch (Tag-73)
    are referenced as precedents.
S3  KW-27-Doppel-Welle constraint with Welle-7 partner is
    documented, including the cross-modul-out-of-band asymmetry
    vs. KW-26.
"""
...
DEFAULT_DOC = "docs/operations/operator-hand-welle-6-eve-recipe-patch.md"
...
REQUIRED_R_PATHS = ["R-W6-A", "R-W6-B", "R-W6-C", "R-W6-D"]
REQUIRED_TP = ["TP-W6-1", "TP-W6-2", "TP-W6-3"]
REQUIRED_VERDICTS = [
    "WELLE-6-VERDICT-CUTOVER-DONE",
    "WELLE-6-VERDICT-NO-GO",
    "WELLE-6-VERDICT-DOPPEL-DECOUPLE",
]
PRE_AUDITOR_STATE_FILE = "state/welle-6-pre-auditor-decision.json"
NATS_SUBSCRIBE_MODE_STATE_FILE = "state/welle-6-subscribe-mode-live.json"
CASCADE_DRIFT_STATE_FILE = (
    "state/welle-6-cascade-into-welle-7-drift.json"
)
IIA_ANCHOR = "IIA-1130"
...
def verify(doc_path: Path) -> VerifyResult:
    ...
    # H5 -- four Welle-6-specific rollback paths.
    ...
    # H6 -- three Welle-6-specific touchpoints.
    ...
    if "§4" not in sections:
        ...
        verdict_rows = [
            ...
            if "->" in line and "WELLE-6-VERDICT-" in line
        ]
    ...
    # H12 -- two Welle-6-specific state-files referenced.
    if NATS_SUBSCRIBE_MODE_STATE_FILE not in text:
        result.hard_fail(
            f"H12: state file path {NATS_SUBSCRIBE_MODE_STATE_FILE} not referenced"
        )
    if CASCADE_DRIFT_STATE_FILE not in text:
        result.hard_fail(
            f"H12: state file path {CASCADE_DRIFT_STATE_FILE} not referenced"
        )

    # S1 -- convergent Welle-6-Day date check.
    if "2026-06-24" not in text or "KW-27" not in text:
        result.soft_warn(
            "S1: doc should reference Welle-6-Day convergent anchor "
            "(KW-27-Mi 2026-06-24)"
        )

    # S2 -- Welle-3-Recipe-Patch (Tag-71) AND Welle-5-Recipe-Patch
    # (Tag-73) precedent references.
    if (
        "Tag-71" not in text
    ...
    # S3 -- KW-27-Doppel-Welle constraint with Welle-7 + out-of-band
    # cross-modul asymmetry.
    if "Doppel-Welle" not in text or "Welle-7" not in text:
        result.soft_warn(
...
def main(argv: List[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Tag-74 verifier for Welle-6 Day-of-Recipe-Patch doc."
    )
    parser.add_argument(
        "--doc",
        type=Path,
    ...
    print(f"verify_welle_6_recipe_patch_doc :: {doc_path}")
    ...
    if result.ok:
        print("VERDICT :: WELLE-6-RECIPE-PATCH-DOC-CLEAN")
        return 0
    print("VERDICT :: WELLE-6-RECIPE-PATCH-DOC-DEFECT")
    return 1
