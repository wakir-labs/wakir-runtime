...
"""Wire the Welle-3 audit-trail-anchor for the persona-engine producer.

Tag-71 Welle-3 State-File Audit-Trail-Anchor-Integration (Tomás,
Cross-Review-Zone-K). Mirror of the Tag-69 Welle-1 / Tag-70 Welle-2
helpers, parameterised to Welle-3 (Bridge-Audit-Writer) bundle.

Cross-coord with Selin (Tag-71 Welle-3 producer):

  * Tomás (this module): hash-construction over the canonical
    Welle-3 bundle, envelope emit. Stdlib-only. No OTS calendar call.
  * Selin (engine.py, welle=3 path): triggers on Bridge-Audit-Writer-
    Cutover-Smoke-Green, calls this helper for the hash-side, writes
    the result into ``state/welle-3.json:audit_trail_anchor``.

The hash construction matches
``emit_manifest_hash_ots_marker.compute_welle_3_audit_anchor_hash``
exactly — the two entry points use the same underlying function via
import. Kept separate from the Tag-69 / Tag-70 ``wire_welle_*``
...
Henrik-IIA-1130 §11-Discipline
-------------------------------

Welle-3 carries an additional discipline: the pre-auditor decision
falls under Henrik's §11-Discipline / IIA-1130. The envelope
exposes ``pre_auditor_signaling_ready`` (True iff the bundle
contains a ``pre_auditor`` part) so the downstream observability
surface can dispatch the pre-auditor-signaling gate without
re-reading the bundle file. **No pre-auditor enforcement here** —
the helper only surfaces readiness, the gate itself lives in
Henrik's Internal-Audit workflow.
...
::
    ...
    python3 tooling/ci/wire_welle_3_audit_trail_anchor.py \\
        --welle-3-rollup    state/welle-3.json \\
        --welle-3-sign-off  state/welle-3-sign-off.json \\
        --welle-3-validation state/welle-3-validation-last-verdict.json \\
        --welle-3-pre-auditor state/welle-3-pre-auditor-decision.json \\
        --envelope-out tooling/ots/markers/welle-3-audit-anchor.json
...
::

  {
    ...
    "bundle_paths": { ... },
    "pre_auditor_signaling_ready": true|false,
    "emitted_at_utc": "<iso>",
    "actor": "<actor>",
    "anchors": { ... },
    "sandbox_boundary": {
...
def _import_emit_helper():
    """Import the Tag-57/Tag-69/Tag-70/Tag-71 OTS-emit module."""
...
ENVELOPE_KIND: str = "welle-3-audit-trail-anchor-producer-envelope"
WELLE_NUMBER: int = 3
...
def build_envelope(
    *,
    ...
    ``pre_auditor_signaling_ready`` is True iff the bundle contains
    a ``pre_auditor`` part. This is the Henrik-IIA-1130 §11-Discipline
    surface for the downstream observability gate.
    """
...
def emit_envelope(
    *,
    ...
    """Load the Welle-3 bundle, compute the anchor, return the envelope.

    This is the importable Python entry-point for tests and for
    Selin's Tag-71 producer (which can in principle import this
    ...
    bundle = emit_module.load_welle_3_bundle(
    ...
    anchor_hash = emit_module.compute_welle_3_audit_anchor_hash(bundle)
...
def main(argv: Iterable[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="wire_welle_3_audit_trail_anchor",
        description=(
            "Compute the Welle-3 audit-trail-anchor SHA-256 from the "
            "sign-off-record bundle and emit a producer-facing "
            "envelope for Selin's Tag-71 persona-engine batch-writer "
            "(Bridge-Audit-Writer). Surfaces Henrik-IIA-1130 §11-"
            "Discipline pre-auditor signaling readiness."
        ),
    )
    parser.add_argument(
        "--welle-3-rollup",
        type=Path,
        required=True,
        help="Path to state/welle-3.json (rollup state-file).",
    )
    parser.add_argument(
        "--welle-3-sign-off",
        type=Path,
        required=True,
        help="Path to state/welle-3-sign-off.json.",
    )
    parser.add_argument(
        "--welle-3-validation",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(optional, but recommended for Henrik-IIA-1130 "
            "§11-Discipline pre-auditor signaling readiness)."
        ),
    )
    parser.add_argument(
        "--envelope-out",
    ...
    if not args.welle_3_rollup.is_file():
        print(
            f"wire_welle_3_audit_trail_anchor: --welle-3-rollup not "
    ...
    if not args.welle_3_sign_off.is_file():
        print(
            f"wire_welle_3_audit_trail_anchor: --welle-3-sign-off not "
            f"a file: {args.welle_3_sign_off}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        envelope = emit_envelope(
            ...
            validation_path=args.welle_3_validation,
            pre_auditor_path=args.welle_3_pre_auditor,
            actor=args.actor,
            now_utc=now_utc,
        )
    except (OSError, json.JSONDecodeError) as exc:
        print(
            f"wire_welle_3_audit_trail_anchor: bundle read/parse "
            f"error: {exc}",
            file=sys.stderr,
        )
        return 1
        ...
        print(
            f"wire_welle_3_audit_trail_anchor: bundle shape error: "
            f"{exc}",
            file=sys.stderr,
        )
        return 1
