...
#
# welle-4-pre-cutover-probe.sh — Phase-3c Pre-Cutover-Sanity-Probe
# for Welle-4 (state_backing, ADR-0066 KW-26 Doppel-Welle).
# -----------------------------------------------------------------
#
# Purpose
#   Executes a non-mutating Pre-Cutover-Sanity-Run against the
#   wakir-pilot live VM (ADR-0058 §Nachtrag) for Welle-4
#   (state_backing).  This probe is the *pre-flight test* of substance
#   against the real substrate ahead of the KW-26 Doppel-Welle cutover
#   window — it confirms that the Welle-4 substance (PR #245 Cutover-
#   Smoke + state-backing CLI snapshot pipeline) meets the GO-criteria
#   *on the actual VM*, not just on the Mira-sandbox hermetic test bed.
#
#   The probe is intentionally distinct from the welle-4 cutover-smoke
#   driver (PR #245): where the smoke *enacts* the Cutover sequence in
#   dry-run-mode against the substrate, the probe only *observes* +
#   *verifies* the system in its Default-State (backend=python for
#   state_backing) before any env-overlay drop-in.
#
...
#   welle-5-pre-cutover-probe.sh for the parallel-run contract.
#
# Authority
...
#
#   AXIS-1  State-Read-Compatibility-Stub — Pre-Cutover snapshot
#           via state-backing-CLI emits non-empty JCS-bytes payload;
#           hash captured for later compare against PHASE_POST.
#   AXIS-2  Welle-4 Cutover-Smoke dry-run (PR #245) emits exit-0 in
#           dry-run-mode against the live VM — substance gate.
#   AXIS-3  BackendDecision-Stream-Health: ≥ 11 BackendDecision audit
#           entries since service start (Tag-38 PR #250 migrate-
#           version baseline) AND state_backing component-line in
#           default backend=python in audit-tail.
#   AXIS-4  Cross-Lang-Parity vs. Pin-Pack anchor: remote state-
#           backing CLI emits canonical snapshot hash identical to
#           in-repo --expected-hash value.
#   AXIS-5  Cross-Modul-Drift-Pre-Check zu Welle-5: lifecycle_state_
#           machine (fsm) overlay is *absent* in Default-State (the
#           symmetric KW-26 isolation contract — Welle-4 probe MUST
#           NOT see the Welle-5 overlay drop-in already wired).
#   AXIS-6  Rollback-Probe: env unset → service restart → engine
...
WAKIR_PROBE_WELLE="${WAKIR_PROBE_WELLE:-4}"
WAKIR_PROBE_COMPONENT="${WAKIR_PROBE_COMPONENT:-state_backing}"
WAKIR_PROBE_PAIR_COMPONENT="${WAKIR_PROBE_PAIR_COMPONENT:-lifecycle_state_machine}"
WAKIR_PROBE_PAIR_OVERLAY_NAME="${WAKIR_PROBE_PAIR_OVERLAY_NAME:-welle-5-lifecycle-state-machine-rust.conf}"
WAKIR_PROBE_OVERLAY_NAME="${WAKIR_PROBE_OVERLAY_NAME:-welle-4-state-backing-rust.conf}"
WAKIR_PROBE_DEFAULT_BACKEND="${WAKIR_PROBE_DEFAULT_BACKEND:-python}"
WAKIR_PROBE_ROLLBACK_ENV_NAME="${WAKIR_PROBE_ROLLBACK_ENV_NAME:-WAKIR_STATE_BACKING_BACKEND}"
...
Pre-Cutover-Sanity-Probe for Phase-3c Welle-4 (state_backing,
ADR-0066 KW-26 Doppel-Welle) against the wakir-pilot live VM via
Mira-Hand-SSH (ADR-0058).

Options:
  --dry-run                Plan probe steps; do not open SSH.
  --quiet                  Suppress non-error stdout chatter.
  --welle <n>              Welle index (default: 4).
  --component <name>       Component name (default: state_backing).
...
# AXIS-1 — State-Read-Compatibility-Stub.
axis_1_state_read_compat() {
    log_step "AXIS-1 state-read-compatibility-stub (component=${WAKIR_PROBE_COMPONENT})"
    local cmd='state-backing-cli snapshot --json --canonical 2>/dev/null | head -c 4096; echo; persona-engine-cli audit-stream-snapshot --tail 5 --json 2>/dev/null | head -c 4096'
    local out rc
...
# AXIS-2 — Welle-4 Cutover-Smoke dry-run (PR #245).
axis_2_smoke_dry_run() {
    log_step "AXIS-2 welle-4-cutover-smoke dry-run (PR #245)"
    local cmd='cd /opt/wakir-runtime && python scripts/phase-3c/welle-4-state-backing-cutover-smoke.py --dry-run --quiet 2>&1 | tail -5; echo "RC=$?"'
    local out rc
    ...
    # RC=0 line indicates green smoke dry-run; non-zero promotes red.
...
# AXIS-3 — BackendDecision-Stream-Health.
axis_3_backend_decision_health() {
    log_step "AXIS-3 backend-decision-stream-health (min=${WAKIR_PROBE_MIN_BACKEND_DECISIONS})"
    local cmd
    ...
    local out rc count
    set +e
    out=$(remote_exec "axis-3-health" "${cmd}") || rc=$?
    rc=${rc:-0}
    if (( rc == 4 )); then
    ...
    AXIS_3_DETAIL="observed=${count} min=${WAKIR_PROBE_MIN_BACKEND_DECISIONS} component_backend=${component_backend:-unknown}"
...
# AXIS-4 — Cross-Lang-Parity vs. Pin-Pack anchor.
axis_4_cross_lang_parity() {
    log_step "AXIS-4 cross-lang-parity (expected=${WAKIR_PROBE_EXPECTED_HASH:-<unset>})"
    ...
    local cmd='state-backing-cli snapshot-hash --canonical-fixture pin-pack-v1 --json 2>/dev/null | head -c 1024'
    local out rc
...
# AXIS-5 — Cross-Modul-Drift-Pre-Check zu Welle-5.
# Symmetric Doppel-Welle isolation: Welle-4's probe verifies that
# the Welle-5 (fsm) overlay is NOT already wired. This is the
# pre-image of the smoke's A7 cross-modul assertion (see ADR-0066
# §Welle-4-5-Risiken).
...
# wrappers:
#
#   * welle-4-pre-cutover-probe.sh AXIS-5 verifies the Welle-5
#     overlay (welle-5-lifecycle-state-machine-rust.conf) is NOT
#     present in Default-State.  If it is, the parallel-Welle
#     isolation contract is already broken and the Welle-4 cutover
#     MUST NOT start.
#
#   * welle-5-pre-cutover-probe.sh AXIS-5 verifies the inverse:
#     the Welle-4 overlay (welle-4-state-backing-rust.conf) is NOT
...
# component swap) in welle-5-pre-cutover-probe.sh.
# -----------------------------------------------------------------
...
main() {
    log_info "welle-4-pre-cutover-probe start ts=${PROBE_TS} host=${WAKIR_PILOT_HOST} dry-run=${DRY_RUN}"
    ...
    axis_1_state_read_compat            || r1=$?
    axis_2_smoke_dry_run                || r2=$?
    axis_3_backend_decision_health      || r3=$?
    axis_4_cross_lang_parity            || r4=$?
