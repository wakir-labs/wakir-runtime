...
"""Mira-Notify Dead-Letter-Queue + Replay CLI (Phase-3c, SRE).

Context
-------

shipped the Mira-Notify Emitter + Receiver substance.
...
The Pre-Mortem-Coverage-Sweep and the Welle-N-Specific
...
..49 plus Cutover-Day) puts the notify-pipeline under
...
*: emitter + receiver substance (the source of the
  dead-letter file).
*: 18 Welle-N alert-rules (the load profile that
  motivates a DLQ-replay path).
* This tool,: closes the dead-letter -> inbox loop.
...
def render_inspect_markdown(report: InspectReport) -> str:
    ...
    lines.append("")
    lines.append("---")
    lines.append("_Generated by `mira-notify-dlq.py`._")
    return "\n".join(lines) + "\n"
