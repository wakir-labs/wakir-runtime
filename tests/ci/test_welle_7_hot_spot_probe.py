...
"""Hermetic tests for the Tag-48 Welle-7 Hot-Spot Probe substrate.

Welle-7 = ``recovery_workflow`` (terminal propagation-target hot-spot;
Recovery-Drill-Live indicator + IIA-1130 Pre-Auditor-Decision-Tracking).
NO downstream propagation -- terminal welle.
...
WORKFLOW_PATH = (
    REPO_ROOT / ".github" / "workflows" / "phase-3c-welle-7-hot-spot-probe.yml"
)
AGGREGATOR_PATH = (
    REPO_ROOT / "tooling" / "ci" / "welle_7_hot_spot_aggregator.py"
)
RUNBOOK_PATH = (
    REPO_ROOT / "docs" / "ci" / "welle-7-hot-spot-probe-runbook.md"
)
...
def _load_aggregator():
    spec = importlib.util.spec_from_file_location(
        "welle_7_hot_spot_aggregator", AGGREGATOR_PATH
    )
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["welle_7_hot_spot_aggregator"] = mod
...
def test_envelope_schema_fields(aggregator):
    ...
    assert envelope["workflow"] == "phase-3c-welle-7-hot-spot-probe"
...
def test_terminal_welle_no_propagation_even_when_red(aggregator):
    """Welle-7 is terminal -- propagation block never fires."""
    env = {
        "CHECK1_STATUS": "green",
        "CHECK2_STATUS": "red",
        "CHECK3_STATUS": "green",
        "CHECK4_STATUS": "green",
        "WELLE_7_RECOVERY_DRILL_LIVE": "red",
    }
    envelope = aggregator.build_envelope(env)
    # Still BLOCK on check-2 red.
    assert envelope["verdict"] == "BLOCK"
    # But NO propagation -- Welle-7 is terminal.
    assert envelope["cross_welle_propagation"] is None


def test_terminal_welle_propagation_constant_empty(aggregator):
    """DOWNSTREAM_PROPAGATION_WELLEN is empty for Welle-7."""
    assert aggregator.DOWNSTREAM_PROPAGATION_WELLEN == ()


def test_terminal_welle_no_propagation_on_yellow(aggregator):
    env = {
        "CHECK1_STATUS": "green",
        "CHECK2_STATUS": "yellow",
        "CHECK3_STATUS": "green",
        "CHECK4_STATUS": "green",
        "WELLE_7_RECOVERY_DRILL_LIVE": "yellow",
    }
    envelope = aggregator.build_envelope(env)
...
def test_notify_event_on_block(tmp_path, aggregator):
    ...
    event = json.loads(notify.read_text(encoding="utf-8").splitlines()[0])
    assert event["kind"] == "welle-7-hot-spot-probe"
...
def test_notify_event_caution_transition_only(tmp_path, aggregator):
    ...
    envelope = aggregator.build_envelope(env)
    assert envelope["verdict"] == "CAUTION"
    assert aggregator._emit_notify_event(envelope, notify, "CLEAR") is True
    assert aggregator._emit_notify_event(envelope, notify, "CAUTION") is False
...
def test_notify_event_clear_silent(tmp_path, aggregator):
    ...
    envelope = aggregator.build_envelope(env)
    assert envelope["verdict"] == "CLEAR"
    assert aggregator._emit_notify_event(envelope, notify, None) is False
...
def test_daily_cron_trigger(workflow_yaml):
    ...
    crons = [item.get("cron") for item in on.get("schedule")]
    # Welle-7 slot: 50 6 * * *
    assert "50 6 * * *" in crons
...
def test_path_filter_covers_substrate(workflow_yaml):
    ...
    required = {
        "tooling/ci/welle_7_hot_spot_aggregator.py",
        ".github/workflows/phase-3c-welle-7-hot-spot-probe.yml",
        "tests/ci/test_welle_7_hot_spot_probe.py",
        "docs/ci/welle-7-hot-spot-probe-runbook.md",
    }
    assert not (required - pr_paths)
    assert not (required - push_paths)


def test_runbook_cross_references():
    text = RUNBOOK_PATH.read_text(encoding="utf-8")
    assert ".github/workflows/phase-3c-welle-7-hot-spot-probe.yml" in text
    assert "tooling/ci/welle_7_hot_spot_aggregator.py" in text
    assert "Henrik" in text and "Pre-Mortem" in text
    assert "IIA-1130" in text
    assert "Recovery" in text or "recovery" in text
    # Welle-7 is terminal; runbook should call that out.
    assert "terminal" in text.lower() or "no downstream" in text.lower()
