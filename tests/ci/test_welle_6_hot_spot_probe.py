...
"""Hermetic tests for the Tag-48 Welle-6 Hot-Spot Probe substrate.

Welle-6 = ``subscribe_loop`` (propagation-target hot-spot;
NATS-Subscribe-Mode-Live-Check indicator). Propagates to Welle-{7}.
...
WORKFLOW_PATH = (
    REPO_ROOT / ".github" / "workflows" / "phase-3c-welle-6-hot-spot-probe.yml"
)
AGGREGATOR_PATH = (
    REPO_ROOT / "tooling" / "ci" / "welle_6_hot_spot_aggregator.py"
)
RUNBOOK_PATH = (
    REPO_ROOT / "docs" / "ci" / "welle-6-hot-spot-probe-runbook.md"
)
...
CHECK_KEYS = (
    ...
    "check_2_nats_subscribe_mode_live_check",
...
def _load_aggregator():
    spec = importlib.util.spec_from_file_location(
        "welle_6_hot_spot_aggregator", AGGREGATOR_PATH
    )
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["welle_6_hot_spot_aggregator"] = mod
...
def test_envelope_schema_fields(aggregator):
    ...
    assert envelope["workflow"] == "phase-3c-welle-6-hot-spot-probe"
...
def test_cross_welle_propagation_fires_to_welle_7(aggregator):
    env = {
        ...
        "CHECK3_STATUS": "green",
        "CHECK4_STATUS": "green",
        "WELLE_6_SUBSCRIBE_MODE_LIVE": "red",
    }
    envelope = aggregator.build_envelope(env)
    assert envelope["verdict"] == "BLOCK"
    prop = envelope["cross_welle_propagation"]
    assert prop is not None
    assert prop["trigger"] == "welle-6-subscribe-mode-live-red"
...
def test_cross_welle_propagation_quiet_on_yellow(aggregator):
    env = {
        ...
        "CHECK3_STATUS": "green",
        "CHECK4_STATUS": "green",
        "WELLE_6_SUBSCRIBE_MODE_LIVE": "yellow",
    }
    envelope = aggregator.build_envelope(env)
...
def test_cross_welle_propagation_requires_both_red(aggregator):
    env = {
        ...
        "CHECK3_STATUS": "green",
        "CHECK4_STATUS": "green",
        "WELLE_6_SUBSCRIBE_MODE_LIVE": "green",
    }
    envelope = aggregator.build_envelope(env)
    assert envelope["verdict"] == "BLOCK"
    assert envelope["cross_welle_propagation"] is None
...
def test_notify_event_on_block(tmp_path, aggregator):
    ...
    assert event["kind"] == "welle-6-hot-spot-probe"
...
def test_workflow_has_five_jobs_in_order(workflow_yaml):
    ...
    expected = [
        ...
        "check-2-nats-subscribe-mode-live-check",
...
def test_daily_cron_trigger(workflow_yaml):
    ...
    # Welle-6 slot: 45 6 * * *
    assert "45 6 * * *" in crons, f"daily-cron not found in: {crons}"
...
def test_path_filter_covers_substrate(workflow_yaml):
    ...
    required = {
        ...
        "tests/ci/test_welle_6_hot_spot_probe.py",
...
def test_runbook_cross_references():
    ...
    assert ".github/workflows/phase-3c-welle-6-hot-spot-probe.yml" in text
    assert "tooling/ci/welle_6_hot_spot_aggregator.py" in text
    assert "Henrik" in text and "Pre-Mortem" in text
    assert "IIA-1130" in text
    assert "NATS" in text or "subscribe" in text.lower()
...
def test_downstream_propagation_wellen_constant(aggregator):
    """Welle-6 propagates to Welle-{7} only."""
