...
the substrate-fence) regresses with a clear hermetic-test failure
...
sandbox (no podman-socket needed).
...
WORKFLOW = REPO_ROOT / ".github" / "workflows" / "build-rust-cli-v907-verify.yml"
...
def _step_present(steps: list[dict], name: str) -> bool:
    return any(s.get("name") == name for s in steps)
...
def test_workflow_name(workflow_yaml: dict) -> None:
    assert workflow_yaml.get("name") == "build-rust-cli-v907-verify"
...
def test_push_trigger_path_filtered(on_block: dict) -> None:
    ...
    assert isinstance(paths, list) and paths, "push trigger must be path-filtered"
    # Must include the crate path so changes to the binary or library
    # trigger a substrate-CI rebuild.
    assert any(
        "persona-engine-v907-verify" in p for p in paths
    ), "push trigger must include the v907-verify crate path"
    # Must include the Containerfile so changes to base-layer pins
    # trigger a rebuild.
    assert any(
        "v907-verify-rust-cli/Containerfile" in p for p in paths
    ), "push trigger must include the v907-verify Containerfile path"


def test_workflow_dispatch_version_tag_input(on_block: dict) -> None:
    wd = on_block["workflow_dispatch"]
    inputs = wd.get("inputs", {})
    assert "version_tag" in inputs, "workflow_dispatch must accept version_tag input"
    assert inputs["version_tag"].get("required") is True


def test_workflow_dispatch_push_input(on_block: dict) -> None:
    wd = on_block["workflow_dispatch"]
    inputs = wd.get("inputs", {})
...
def test_no_extra_permission_scopes(workflow_yaml: dict) -> None:
    ...
    # scopes (actions:write, security-events:write, etc.).
...
def test_checkout_step_present(build_steps: list[dict]) -> None:
    _step_index(build_steps, "Checkout")


def test_cargo_build_step_present(build_steps: list[dict]) -> None:
    # Substrate-CI smoke: cargo build BEFORE buildah build.
    _step_index(build_steps, "Cargo build (substrate-CI smoke, --release)")


def test_cargo_build_uses_release_flag(build_steps: list[dict]) -> None:
    idx = _step_index(build_steps, "Cargo build (substrate-CI smoke, --release)")
    run = build_steps[idx].get("run", "")
    assert "cargo build --release" in run, "cargo build must use --release"
    assert "-p persona-engine-v907-verify" in run, (
        "cargo build must target the v907-verify crate explicitly"
    )
    assert "--bin wakir-v907-verify" in run, (
        "cargo build must target the wakir-v907-verify binary explicitly"
    )


def test_buildah_step_present(build_steps: list[dict]) -> None:
    _step_index(build_steps, "Build image with buildah")
...
def test_cosign_installer_present(build_steps: list[dict]) -> None:
    # sigstore/cosign-installer@v3 must be present (parity with
    # build-wakir-persona-engine.yml).
...
def test_cosign_sign_step_present(build_steps: list[dict]) -> None:
    _step_index(build_steps, "Sigstore-keyless sign")
...
def test_version_tag_fence_exits_64_on_drift(workflow_text: str) -> None:
    # The fence step must exit 64 on mismatch (parity with
    # build-wakir-persona-engine.yml). Exit 64 is the
    # operator-friendly "usage error" signal.
    fence_block_start = workflow_text.find(
        "Resolve effective version_tag + push-flag"
    )
    assert fence_block_start != -1, "fence step missing"
    # Find ``exit 64`` within ~80 lines of the fence step.
    fence_chunk = workflow_text[fence_block_start : fence_block_start + 4000]
    assert "exit 64" in fence_chunk, "fence step does not exit 64 on drift"
...
def test_digest_artifact_emit_step_present(build_steps: list[dict]) -> None:
    _step_index(build_steps, "Emit sha256-digest as artifact")


def test_digest_artifact_upload_step_present(build_steps: list[dict]) -> None:
    _step_index(build_steps, "Upload digest artifact")


def test_digest_artifact_conditional(build_steps: list[dict]) -> None:
...
def test_digest_artifact_uses_upload_v4(build_steps: list[dict]) -> None:
    idx = _step_index(build_steps, "Upload digest artifact")
...
def test_rust_base_digest_resolver_present(build_steps: list[dict]) -> None:
    _step_index(build_steps, "Resolve rust:1.85-slim-bookworm base-layer digest")


def test_distroless_base_digest_resolver_present(build_steps: list[dict]) -> None:
    _step_index(
        build_steps, "Resolve distroless/cc-debian12:nonroot base-layer digest"
    )
...
def test_skopeo_crane_cross_check_in_rust_resolver(
    ...
    # inconsistency guard, parity with
    # build-wakir-persona-engine.yml).
    idx = _step_index(
    ...
    assert "exit 2" in run, (
        "rust base-layer resolver must exit 2 on skopeo/crane disagreement"
    )
...
def test_containerfile_present_on_disk() -> None:
    cf = REPO_ROOT / "infra" / "v907-verify-rust-cli" / "Containerfile"
    assert cf.is_file(), f"missing Containerfile: {cf}"
