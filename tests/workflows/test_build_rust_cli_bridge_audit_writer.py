...
WORKFLOW = (
    ...
    / ".github"
    / "workflows"
    / "build-rust-cli-bridge-audit-writer.yml"
)
CONTAINERFILE = (
    REPO_ROOT
    / "infra"
...
def test_workflow_name(workflow_yaml: dict) -> None:
    assert (
        ...
        == "build-rust-cli-bridge-audit-writer"
    )
...
def test_push_trigger_path_filtered(on_block: dict) -> None:
    ...
    assert any(
        "persona-engine-bridge-audit-replay" in p for p in paths
    ), (
        "push trigger must include the bridge-audit-replay crate path "
        "(the binary ships from this crate)"
    )
    # Must include the Containerfile so changes to base-layer pins
    # trigger a rebuild.
    assert any(
        "bridge-audit-writer-rust-cli/Containerfile" in p for p in paths
    ), "push trigger must include the bridge-audit-writer Containerfile path"
...
def test_cargo_build_targets_bridge_audit_replay_crate_and_writer_binary(
    ...
    assert "-p persona-engine-bridge-audit-replay" in run, (
        "cargo build must target the persona-engine-bridge-audit-replay "
        "crate explicitly (the writer binary lives in this crate, "
        "alongside the sibling replay_cli)"
    )
    assert "--bin wakir-persona-engine-bridge-audit-writer" in run, (
        "cargo build must target the wakir-persona-engine-bridge-audit-"
        "writer binary explicitly"
    )
...
def test_digest_artifact_name_matches_image_name(
    ...
    assert name == "wakir-persona-engine-bridge-audit-writer-digest", (
        "digest artefact name must match the canonical convention "
        "wakir-persona-engine-<component>-digest; got "
        f"{name!r}"
    )
...
def test_containerfile_entrypoint_targets_writer_binary() -> None:
    txt = CONTAINERFILE.read_text(encoding="utf-8")
    assert (
        ...
        "binary path /usr/local/bin/wakir-persona-engine-bridge-audit-writer"
    )
