"""Tests for the Branch-Protection Required-Checks inventory doc.

Hermetic, stdlib-only. Validates both:
 - the live doc shipped in `docs/ci/branch-protection-required-checks.md`
...
DOC_PATH = REPO_ROOT / "docs" / "ci" / "branch-protection-required-checks.md"
HELPER_PATH = (
    REPO_ROOT
    / "tooling"
    / "ci"
...
class FrontmatterTests(unittest.TestCase):
    ...
    def test_frontmatter_status_active(self):
        self.assertIn('status: "active"', self.text)
...
class StatusTableTests(unittest.TestCase):
    ...
    def test_row_count_matches_helper_expectation(self):
        self.assertEqual(len(self.rows), HELPER.EXPECTED_CHECK_COUNT)
    ...
    def test_all_rows_have_known_status(self):
        for r in self.rows:
            status = r["status"].upper()
            self.assertTrue(
                status.startswith("ACTIVE") or status.startswith("PENDING"),
                f"unknown status cell: {r['status']!r}",
            )

    def test_every_workflow_file_exists(self):
        for r in self.rows:
            wf = HELPER.extract_backtick(r["workflow"])
            self.assertTrue((REPO_ROOT / wf).is_file(), f"missing workflow {wf}")
...
class SandboxBoundaryTests(unittest.TestCase):
    ...
    def test_sandbox_marked(self):
        self.assertIn("sandbox", self.s6.lower())
