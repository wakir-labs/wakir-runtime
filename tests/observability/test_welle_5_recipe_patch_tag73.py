...
"""Tag-73 hermetic tests for the Welle-5 Day-of-Recipe-Patch
substrate (Operator-Hand companion doc to the Tag-66 Eve-Recipe,
following the Welle-3 Tag-71-Patch precedent).

Layout:
  * Tests A1..A8 exercise the doc itself
    (docs/operations/operator-hand-welle-5-eve-recipe-patch.md).
  * Tests B1..B14 exercise the verifier
    (tooling/ci/verify_welle_5_recipe_patch_doc.py) -- including
...
DOC_PATH = (
    ...
    / "docs"
    / "operations"
    / "operator-hand-welle-5-eve-recipe-patch.md"
)
VERIFIER_PATH = (
    REPO_ROOT / "tooling" / "ci" / "verify_welle_5_recipe_patch_doc.py"
)


def _load_verifier():
    spec = importlib.util.spec_from_file_location(
        "verify_welle_5_recipe_patch_doc", VERIFIER_PATH
    )
...
def test_a2_doc_has_yaml_frontmatter_with_required_keys():
    ...
    assert 'tag: "tag-73"' in fm, "tag value must be tag-73"
...
def test_a5_doc_lists_four_welle_5_rollback_paths():
    txt = DOC_PATH.read_text(encoding="utf-8")
    for r in ("R-W5-A", "R-W5-B", "R-W5-C", "R-W5-D"):
        assert f"### {r} --" in txt, f"rollback path {r} missing"


def test_a6_doc_lists_three_ar_hand_touchpoints():
    txt = DOC_PATH.read_text(encoding="utf-8")
    for tp in ("TP-W5-1", "TP-W5-2", "TP-W5-3"):
        assert f"### {tp} --" in txt, f"touchpoint {tp} missing"


def test_a7_doc_explicitly_names_both_hot_spot_axes():
    txt = DOC_PATH.read_text(encoding="utf-8")
    assert "fsm-phantom" in txt, "fsm-phantom axis name missing"
    assert "capability-token-rotation" in txt, (
        "capability-token-rotation axis name missing"
    )


def test_a8_doc_references_kw26_doppel_welle_and_welle_4_partner():
    txt = DOC_PATH.read_text(encoding="utf-8")
    assert "KW-26" in txt, "KW-26 anchor missing"
    assert "Doppel-Welle" in txt, "Doppel-Welle wording missing"
    assert "Welle-4" in txt, "Welle-4 partner reference missing"
...
def test_b6_verifier_detects_missing_rollback_path(tmp_path):
    mod = _load_verifier()
    txt = DOC_PATH.read_text(encoding="utf-8")
    mutated = txt.replace("### R-W5-D --", "### R-DELETED --", 1)
    p = tmp_path / "patch.md"
    p.write_text(mutated, encoding="utf-8")
    result = mod.verify(p)
    assert not result.ok
    assert any("R-W5-D" in f and "H5" in f for f in result.hard_failures)
...
def test_b9_verifier_detects_short_verdict_table(tmp_path):
    ...
    # Replace verdict rows except the first three with empty.
    # Each row matches `| ... -> ``WELLE-5-VERDICT-...``...|`
    lines = txt.splitlines()
    seen = 0
    out = []
    for line in lines:
        if "WELLE-5-VERDICT-" in line and "->" in line:
            seen += 1
            if seen > 3:
                continue
        out.append(line)
...
def test_b10_verifier_detects_missing_hot_spot_axis(tmp_path):
    ...
    mod = _load_verifier()
    txt = DOC_PATH.read_text(encoding="utf-8")
    mutated = txt.replace("fsm-phantom", "REMOVED-AXIS-A")
    p = tmp_path / "patch.md"
    p.write_text(mutated, encoding="utf-8")
    result = mod.verify(p)
    assert not result.ok
    assert any(
        "H11" in f and "fsm-phantom" in f for f in result.hard_failures
    )


def test_b11_verifier_detects_missing_axis_state_file(tmp_path):
    """Strip capability-token-rotation-drift state-file ref, verify
    H12 fires."""
    mod = _load_verifier()
    txt = DOC_PATH.read_text(encoding="utf-8")
    mutated = txt.replace(
...
def test_b12_verifier_cli_exit_zero_on_real_doc():
    ...
    assert "WELLE-5-RECIPE-PATCH-DOC-CLEAN" in proc.stdout
...
def test_b14_verifier_constants_match_doc_substrate():
    ...
    assert (
        mod.PRE_AUDITOR_STATE_FILE == "state/welle-5-pre-auditor-decision.json"
    )
    assert (
        mod.FSM_PHANTOM_STATE_FILE
        == "state/welle-5-fsm-phantom-detection.json"
    )
    assert (
        mod.CAP_TOKEN_DRIFT_STATE_FILE
        == "state/welle-5-capability-token-rotation-drift.json"
    )
    assert mod.IIA_ANCHOR == "IIA-1130"
    assert mod.HOT_SPOT_AXES == ["fsm-phantom", "capability-token-rotation"]
    ...
    assert mod.REQUIRED_TP == ["TP-W5-1", "TP-W5-2", "TP-W5-3"]


def test_b15_doc_documents_convergent_date_anchor():
    """The Welle-5-Day date is convergent (unlike Welle-3 which is
    fluid). Doc must reference KW-26-Mi 2026-06-17 anchor."""
    txt = DOC_PATH.read_text(encoding="utf-8")
    assert "2026-06-17" in txt, "Welle-5-Day convergent anchor must be referenced"
    assert "KW-26" in txt


def test_b16_doc_lists_helper_substrate_paths():
    """§7 Helper-Substrat must reference the Tag-73 verifier path
    and the Tag-73 test path -- doc-to-substrate closure."""
    txt = DOC_PATH.read_text(encoding="utf-8")
    assert "tooling/ci/verify_welle_5_recipe_patch_doc.py" in txt
    assert "tests/observability/test_welle_5_recipe_patch_tag73.py" in txt
    assert "tooling/ci/welle_5_hot_spot_aggregator.py" in txt


def test_b17_doc_references_welle_3_patch_as_precedent():
    """Welle-5-Patch must reference the Welle-3-Patch (Tag-71) as
    its template precedent -- this is a hard requirement of the
    Tag-73 brief."""
    txt = DOC_PATH.read_text(encoding="utf-8")
