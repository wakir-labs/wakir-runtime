...
Splits cleanly from ``test_cli.py``: that file pins 's JSON-only
...
* Block-end "Quorum conclusion" section names the threshold
  explicitly so the operator review-reader does not have to know the
  policy enum.
"""
...
from wat.anchor.external_verifier.cli import main as cli_main
from wat.anchor.external_verifier.poles import HttpResponse

from tests.wat.external_verifier.fixtures import RECEIPT_948183_BYTES
...
def test_cli_text_mode_default_is_json_for_backwards_compat(tmp_path, capsys):
    """contract: omitting --output-format yields JSON, not text."""
    receipt = _make_receipt(tmp_path)
    rc = cli_main(
...
# ---------------------------------------------------------------------------
# --save-witnesses (substantive: persists alongside text or JSON)
# ---------------------------------------------------------------------------
...
def test_cli_text_mode_witness_capture_replay_renders(tmp_path, capsys):
    ...
    from wat.anchor.external_verifier.cli import _render_witness_capture_text
