...
def test_aggregator_signed_manifest_verifies_through_real_verifier(tmp_path):
    ...
    verifier end-to-end ( closure block).

    The substrate hand-signed deep copies of fixture
    manifests because the aggregator did not yet emit signed
    manifests on its own. With in place, the aggregator's own
    ...
    # this test focused on substance (production-side signing
    ...
    # path is regenerated by the aggregator.
    on_disk = json.loads(out.read_text(encoding="utf-8"))
