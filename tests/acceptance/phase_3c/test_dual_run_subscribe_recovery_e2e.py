...
"""Phase-3c dual-run wave-6+7 E2E acceptance — ``subscribe_loop`` +
``recovery_workflow`` parallel cutover (calendar week 27).

Anchors
-------

- ADR-0066 §Beschluss — dual-run wave calendar week 27: wave 6 + wave 7 parallel.
- ADR-0066 §Verifikations-Plan §"wave 6+7 calendar week 27" — Low-Coupling-
  ...
  modul of all seven (it reads every prior wave's substrate during
  restart-point reconstruction).
- ADR-0065 §wave-Ende-Acceptance §WE-1...WE-4 — fires after wave 7
  completes. dual-run wave-6+7 telescopes the wave 7 AC + WE-1...WE-4
  into the same cutover-week.
- ``test_subscribe_loop_e2e.py`` + ``test_welle_7_recovery_
  workflow_e2e.py`` — per-wave sister files.

dual-run wave character — stateful-loop-paar
-------------------------------------------

wave 6+7 closes Phase-3c. After this cutover, all 7 moduln are on
rust-default. The dual-run wave character:

1. ``subscribe_loop`` holds NATS-subscription state (subscription-
   cursor, connection-keepalive). Bug-42-Lessons-Learned hot
   substrate.
2. ``recovery_workflow`` is the cross-modul orchestrator — by calendar week 27,
...
3. The *pair* coupling is moderate: subscribe_loop delivers
   ...
   touchpoint as directly as wave 4+5 did.

DW-AC-1 + DW-AC-3 + DW-AC-5 carry the dominant gate-weight here;
DW-AC-2 + DW-AC-4 are present but lower-emphasis than wave 4+5.
"""
...
def test_doppel_welle_6_7_dw_ac_1_both_moduln_boot_rust(
    ...
    test_recovery_workflow_e2e.py for the wave-Ende-Acceptance
    gates).
    """
...
def test_doppel_welle_6_7_dw_ac_1_boot_failure_blocks(
    ...
    wave 6 has the Bug-42-replay-coverage substrate (subscribe-mode-
...
def test_doppel_welle_6_7_dw_ac_3_rollback_subscribe_partner_stays_rust(
    ...
    by the per-wave wave 6 Bug-42-Lessons-Learned coverage that's
...
def test_doppel_welle_6_7_dw_ac_3_rollback_recovery_partner_stays_rust(
    ...
    particularly costly because it's the last wave and a rollback
...
def test_doppel_welle_6_7_dw_ac_4_cross_modul_stress_test_green(
    ...
    recovery-restart-points. References the engineering zone substrate
    (Phase-2-Acceptance-Gate-Erweiterung). Per ADR-0066 §Verifikations-
    Plan §"wave 6+7", Standard-Acceptance applies — the stress-load
    is the baseline (not the wave 4+5-enhanced level).
    """
...
def test_doppel_welle_6_7_dw_ac_4_failure_blocks(
    ...
    wave 6+7-specific failure-source: NATS-reconnect-storm during
...
def test_doppel_welle_6_7_dw_ac_5_backend_decision_audit_two_records(
    ...
    Post-wave 7 the audit-trail closes Phase-3c; internal-audit (Zone-N)
    cross-references this audit-record-pair against the per-wave audit-
    trail for the WE-3 wave-Ende-gate (audit-compliance check).
    """
...
def test_doppel_welle_6_7_dw_ac_5_missing_recovery_record_blocks(
    ...
    wave 6+7, the audit-trail for the cutover is incomplete).
    """
...
def test_doppel_welle_6_7_dw_ac_5_wrong_target_backend_blocks(
    ...
    A target_backend other than ``rust`` on wave 7 means recovery_
...
# ---------------------------------------------------------------------------
# dual-run wave-6+7 substrate sanity — wave-Ende-Acceptance hand-off.
# ---------------------------------------------------------------------------
...
def test_doppel_welle_6_7_welle_ende_acceptance_handoff() -> None:
    """dual-run wave-6+7 closes Phase-3c. After this cutover passes all
    DW-AC-1...DW-AC-5 *and* the per-wave wave 7 AC-1...AC-5, the
    wave-Ende-Acceptance lane (WE-1...WE-4, ADR-0065 §wave-Ende-
    Acceptance) fires.

    WE-1: Container-Image-Tag ``0.7.0-rust`` als Production-Default.
    WE-2: Quadlet-Default-ENV-Flags alle 7 auf ``rust``.
    WE-3: internal-audit-Compliance-Check GREEN.
    WE-4: ``wirelang/persona_engine_py_legacy/`` archived (4-Wochen-
    Reserve).

    The WE-1...WE-4 assertions live in test_welle_7_recovery_workflow_
    e2e.py (per-wave file, parent skeleton). This dual-run wave file
    leaves them as the per-wave responsibility — DW-AC-1...DW-AC-5 are
    the dual-run wave-specific value-add, not a WE-1...WE-4 duplicate.
    """
...
def test_doppel_welle_6_7_cmd_ac_6_7_1_byte_drift_blocks(
    ...
    wave 6+7-specific drift-source: Rust-side serde implementation
...
#
# Singleton oracle-set: only python-python-baseline. The wave 4+5
# mid-dual-run wave hypothetical (python-rust-wave N-only) is
# operationally unreachable for wave 6+7 because the subscribe/
...
def test_doppel_welle_6_7_cmd_ac_6_7_2_resubscribe_trigger_wire_form_parity(
    ...
    * R4 — Re-subscribe-with-fast-forward (skip-to-tip).
    ...
    Rust-Rust trigger wire-form must match the pre-dual-run wave-6+7
...
def test_doppel_welle_6_7_cmd_ac_6_7_2_python_baseline_drift_blocks(
    ...
    This is the dominant regression-class for the wave 6+7 Doppel-
    wave: the new Rust-Rust trigger wire-form produces a hash that
...
def test_doppel_welle_6_7_cmd_ac_6_7_2_oracle_set_singleton_anchored_to_adr_0066() -> None:
    ...
    The wave 4+5 mid-dual-run wave hypothetical (python-rust-wave N-
    only) does NOT apply to wave 6+7 because the subscribe/recovery
...
# restart-points. References the engineering zone substrate (Phase-2-Acceptance-
...
def test_doppel_welle_6_7_cmd_ac_6_7_3_consistency_floor_anchored_to_adr_0066() -> None:
    ...
    Shared with wave 4+5 CMD-AC-3 (same constant
...
#
# Same gate-shape as wave 4+5 CMD-AC-4 plus the wave 6+7-specific
# risk surface: a recovery_workflow rollback delays Phase-3c-Ende by
# +1 week (wave 7 is the closing wave).
# ---------------------------------------------------------------------------
...
def test_doppel_welle_6_7_cmd_ac_6_7_4_atomic_flip_subscribe_loop_no_delay(
    ...
    loop is wave 6 (not the closing wave) - Phase-3c-Ende-Tag is
    unaffected.
    """
...
def test_doppel_welle_6_7_cmd_ac_6_7_4_atomic_flip_recovery_workflow_one_week_delay(
    ...
    The closing-wave rollback impact-surface: wave 7 is the last
    cutover-wave, so rolling it back delays Phase-3c-Ende by +1
...
def test_doppel_welle_6_7_cmd_ac_6_7_4_threshold_anchored_to_adr_0066() -> None:
    ...
    Shared with wave 4+5 CMD-AC-4 (same constant
