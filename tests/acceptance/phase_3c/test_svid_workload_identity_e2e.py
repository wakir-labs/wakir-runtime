...
"""Phase-3c cutover wave 2 E2E acceptance — ``svid_workload_identity``.

Anchors
-------

- ADR-0065 §Verifikations-Plan §"Vorgeschlagene Reihenfolge" — wave 2
...
- ``infra/spire/`` (existing SPIRE federation substrate) — the workload-
  identity surface this wave re-implements in Rust.
...
windows). wave 2 after wave 1 in the risk-ascending order.
...
def test_welle_2_ac_1_drift_detected_rejects(
    ...
    detector must catch it before wave 2 cutover declares success.
    """
...
def test_welle_2_ac_3_bug_rate_zero_s0_s1() -> None:
    """AC-3: 0 S0/S1 issues during wave 2 Beobachtungs-Woche."""
...
def test_welle_2_ac_4_cross_review_consensus(mocked_cross_review) -> None:
    """AC-4: the org zone-protokolliert wave 2 Cross-Review-Session consent."""
...
def test_welle_2_ac_4_missing_persona_blocks(mocked_cross_review) -> None:
    ...
    wave 2 is SPIRE-substrate-relevant → the infrastructure zone (substrate-owner) must
    consent. Withholding the infrastructure zone's consent must block the gate.
    """
...
def test_welle_2_ac_5_v907_pin_validation_full_pass() -> None:
    """AC-5: V-907 Pin-Validation 100% post-wave 2 cutover.
...
# ---------------------------------------------------------------------------
# wave 2 substrate sanity — wave 1 stays on rust, only wave 2 flips.
# ---------------------------------------------------------------------------


def test_welle_2_quadlet_state_welle_1_and_2_rust(mocked_quadlet_env) -> None:
    """wave 2 Quadlet-state: ``v907_verify`` + ``svid_workload_identity``
    on rust, remaining 5 moduln on python.

    wave 2 is the first opportunity to verify the *mixed-backend
    persistence* requirement: wave 1's flip must not regress when
    wave 2 flips its own ENV-flag.
    """
    ...
    assert rust_count == 2 and python_count == 5, (
        f"wave 2 mixed-state must have exactly 2 rust + 5 python; "
...
def test_welle_2_spire_agent_consistency_across_rotation() -> None:
    """wave 2 specific: SPIRE-SVID-rotation consistency.
